...
/** The slice of Grok Build's config.toml these assertions read. */
interface GrokBuildToml {
...
describe("Grok Build config", () => {
  it("builds the expected provider TOML", () => {
    ...
    const parsed = parseToml(config) as unknown as GrokBuildToml;
...
`;
    ...
    const parsed = parseToml(updated) as unknown as GrokBuildToml;
  ...
  it("renames the selected profile without leaving the old table behind", () => {
    ...
    const parsed = parseToml(renamed) as unknown as GrokBuildToml;
