...
describe("grokBuildProviderPresets", () => {
  ...
  it("excludes providers deliberately kept off the roster", () => {
    ...
    const excluded = [
      ...
      "Nvidia",
      "AtlasCloud",
      // Upstream has had grok-4.5 since 2026-08, but whether the subscription
      // gateway should include it is still a pending product decision;
      // for now it stays deliberately excluded (no longer because "upstream lacks Grok").
      "OpenCode Go",
    ];
