...
export const opencodeProviderPresets: OpenCodeProviderPreset[] = [
  ...
  },
  {
    ...
    // across both series; the Auto smart-routing call ID is tc-code-latest. The
    // endpoint is OpenAI-compatible (the official quickstart 1823/130119 has not
    // published a dedicated OpenCode onboarding page, so this is registered under the
    // tool-agnostic /plan/v3 endpoint, with the lineup following the docs). kimi-k2.5
    // is marked as officially retired on 2026-08-31 and is excluded; minimax-m2.5
    // isn't in the plan doc's table but is listed in /plan/v3/models and tested
    // working with a real key (2026-08-31), so it's included as-is
  ...
  },
  {
    ...
    // revision): the Auto call ID is auto (different from the domestic personal
    // edition's tc-code-latest), and the lineup differs from the domestic one (no
    // GLM-5/5.1/Hy3, plus GLM-5.2/MiniMax-M3). The endpoint uses the tencentcloudmaas.com
    // domain specified by the international-site docs; keys are separate per site and
    // don't work across sites
  ...
  },
  {
    ...
    // kimi-k2.5 is marked as officially retired on 2026-08-31 and is excluded;
    // minimax-m2.5 was removed from the model list but tested working with a real key
    // (2026-08-31), so it's included as-is
  ...
  },
  {
    ...
    // starting 2026-07-13): the official OpenCode onboarding page confirms
    // /v2/tokenplan/personal + @ai-sdk/openai-compatible; the lineup matches the six
    // models in the Token Plan main doc's 2026-08-14 revision (ernie-5.1 retired on
    // 8/20 and excluded)
  ...
  // Pay-as-you-go goes through the OpenAI-compatible layer (/compatible-mode/v1);
  // Token Plan's official address uses the Anthropic protocol, with one more /v1
  // segment than Claude Code's (the AI SDK anthropic convention).
  {
    name: "QwenCloud",
