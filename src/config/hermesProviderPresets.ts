...
export const hermesProviderPresets: HermesProviderPreset[] = [
  ...
  // Field mapping: env.ANTHROPIC_BASE_URL → base_url; env.ANTHROPIC_AUTH_TOKEN → api_key;
  // apiFormat "anthropic" (default) → api_mode "anthropic_messages";
  // apiFormat "openai_chat" → api_mode "chat_completions";
  // ANTHROPIC_MODEL / DEFAULT_HAIKU / SONNET / OPUS_MODEL are deduplicated and pushed into models[].
  {
    name: "Zhipu GLM",
  ...
  },
  {
    ...
    // general and Hy series share the same endpoint and API key, with models merged
    // across both series; the Auto smart-routing call ID is tc-code-latest. Tencent has
    // not published a dedicated Hermes onboarding page, so this is registered under the
    // tool-agnostic /plan/v3 OpenAI Chat endpoint (matches the chat_completions mode
    // used by marketplace line 1300/80643). kimi-k2.5 was officially retired on 2026-08-31
    // and is excluded; minimax-m2.5 tested working with a real key (2026-08-31) and is
    // included as-is
  ...
  },
  {
    ...
    // removed from the docs but tested working with a real key (2026-08-31), so it's
    // included as-is
  ...
  },
  {
    ...
    // 5-Turbo, Kimi-K2.6, MiniMax-M2.7)
  ...
  },
  {
    ...
    // (resource scheduling scope is Global), Auto model only
  ...
  },
  {
    ...
    // starting 2026-07-13): the official Hermes onboarding page confirms
    // /v2/tokenplan/personal, defaulting to deepseek-v4-pro (its api_mode is written as
    // "openai_messages", which this repo's OpenAI Chat endpoint convention maps
    // uniformly to chat_completions); the lineup matches the six models in the Token
    // Plan main doc's 2026-08-14 revision
  ...
  // Both QwenCloud and Token Plan use anthropic_messages, and the address doesn't
  // include /v1 (this matches the official Hermes docs, but differs from
  // OpenCode / OpenClaw — don't copy across).
  {
    name: "QwenCloud",
