...
const piProviderPresetDefinitions: PiProviderPreset[] = [
  ...
  // A separate site from the domestic entry above: domain, console, and API keys are not interchangeable.
  // Both pay-as-you-go and Token Plan go through the OpenAI-compatible layer (/compatible-mode/v1).
  {
    name: "QwenCloud",
  ...
  // ===== Tencent Cloud Token Plan (subscription package, product line 1823/1300) =====
  // Shares its origin with the Tencent presets in the six apps claude/codex/opencode/hermes/openclaw:
  // the personal edition uses api.lkeap.cloud.tencent.com/plan domestically, and
  // tencentmaas.com/plan elsewhere; the two sites' keys are not interchangeable, and
  // dual-region candidates (domestic -> Singapore, international -> Guangzhou) only
  // exist for the enterprise package. The Pi preset only carries the default region's
  // baseUrl (PiProviderPreset has no endpointCandidates field); users who need the
  // second region must change baseUrl manually in the UI.
  {
  ...
  // ===== Tencent Cloud TokenHub (pay-as-you-go API marketplace, product line 1823/1300, /v1 endpoint) =====
  // A separate product line from the Token Plan subscription line (/plan endpoint): keys are not interchangeable, and endpoints differ.
  // The model list is taken from the official "language model" listing (domestic 130051 / international 78934), and only includes
  // general-purpose language models that support Function Calling; specialized models such as translation (hy-mt2) and
  // role-play (hy-role) are not included (they cannot be used for a coding agent).
  {
