...
use ai_manager_lib::{
    get_codex_auth_path, get_codex_config_path, read_json_file, write_codex_live_atomic, AppError,
...
fn settings_path(_home: &Path) -> PathBuf {
    product_data_dir().join("settings.json")
}
...
"#;
    ...
    ProviderService::import_default_config(&state, AppType::Codex).expect("import codex default");
...
"#
    ...
    // v3.7.0+: use the unified MCP structure.
    ...
    let new_config_text = new_provider
        ...
        .and_then(|v| v.as_str())
        .unwrap_or_default();
    // The provider config must be present in the live file.
    // Note: the live file also contains whatever the MCP sync wrote.
    assert!(
    ...
    let legacy_auth_value = legacy
        ...
        .and_then(|v| v.as_str())
        .unwrap_or("");
    // Backfill: before switching, the live config is written back into the current provider.
    // This preserves manual edits the user made in the live file.
    assert_eq!(
...
fn switch_provider_updates_claude_live_and_state() {
    ...
    let settings_path = ai_manager_lib::get_claude_settings_path();
    ...
    // v3.7.0+ stores data in SQLite rather than config.json.
    // Verify it was persisted to the database.
    let db_path = product_data_dir().join("app.db");
    assert!(
        db_path.exists(),
        "switching provider should persist to app.db"
    );

    // Verify the current provider was updated.
    let current_id = app_state
        .db
        .get_current_provider(AppType::Claude.as_str())
        .expect("get current provider");
...
fn switch_provider_codex_missing_auth_returns_error_and_keeps_state() {
    ...
    let current_id = app_state
        ...
        .get_current_provider(AppType::Codex.as_str())
        .expect("get current provider");
    // After a failed switch, `current` may already read "invalid" because the database
    // write happens before validation; the live-config write failed, so the state should
    // roll back. Note: this depends on the concrete switch_provider implementation.
    assert!(
...
fn import_refuses_live_config_under_proxy_takeover() {
    ...
    // Taken-over Codex live: auth is the PROXY_MANAGED placeholder, not the user's real config.
