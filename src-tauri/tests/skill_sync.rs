...
#[path = "support.rs"]
mod support;
use support::{create_test_state, ensure_test_home, product_data_dir, reset_test_fs, test_mutex};
...
fn import_from_apps_does_not_rewrite_selected_app_directory() {
    ...
    let ssot_skill_dir = product_data_dir().join("skills").join("codex-skill");
...
fn sync_to_app_removes_disabled_and_orphaned_ssot_symlinks() {
    ...
    let ssot_dir = product_data_dir().join("skills");
...
fn uninstall_skill_creates_backup_before_removing_ssot() {
    ...
    let ssot_skill_dir = product_data_dir().join("skills").join("backup-skill");
...
fn restore_skill_backup_restores_files_to_ssot_and_current_app() {
    ...
    let ssot_skill_dir = product_data_dir().join("skills").join("restore-skill");
...
fn delete_skill_backup_removes_backup_directory() {
    ...
    let ssot_skill_dir = product_data_dir()
        .join("skills")
