...
use support::{
    ...
    ensure_test_home, product_data_dir, reset_test_fs, test_mutex,
};
...
fn sync_claude_provider_writes_live_settings() {
    ...
    // Confirm the provider in the SSOT picked up the latest content too.
    let updated = config
    ...
    // Also confirm the write landed under the test HOME.
    assert!(
...
fn sync_codex_provider_writes_config_without_touching_auth() {
    ...
    // Note: since v3.7.0 the MCP sync is handled by McpService and is no longer triggered
    // by a provider switch. Switching a Codex provider only writes config.toml; auth.json
    ...
    let auth_path = ai_manager_lib::get_codex_auth_path();
...
fn sync_codex_provider_with_config_only_token_backfills_auth() {
    // P2-2 regression: when the stored provider's token only lives in config.toml's
    // experimental_bearer_token, the sync path must extract it from the live config and write
    // it back to the stored auth.OPENAI_API_KEY; otherwise the next sync loses the token
    // between a cleaned config and an empty auth.
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
...
requires_openai_auth = true
"#;
    ai_manager_lib::write_codex_live_atomic(&legacy_auth, Some(legacy_config))
...
"#
    ...
    let toml_text =
        fs::read_to_string(ai_manager_lib::get_codex_config_path()).expect("read config.toml");
...
fn sync_enabled_to_codex_writes_enabled_servers() {
    ...
    // Simulate Codex being installed/initialized: ~/.codex exists.
    let path = ai_manager_lib::get_codex_config_path();
    ...
    ai_manager_lib::sync_enabled_to_codex(&config).expect("sync codex");
...
fn sync_enabled_to_codex_preserves_non_mcp_content_and_style() {
    ...
    // Seed a config.toml with a top-level comment and non-MCP keys.
    let path = ai_manager_lib::get_codex_config_path();
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).expect("create codex dir");
    }
...
"#;
    ...
    // Enable one MCP entry to trigger an incremental write.
    let mut config = MultiAppConfig::default();
    config.mcp.codex.servers.insert(
        "echo".into(),
        json!({
    ...
    let text = fs::read_to_string(&path).expect("read config.toml");
    // The top-level comment and non-MCP keys must be preserved.
    assert!(
...
fn sync_enabled_to_codex_migrates_erroneous_mcp_dot_servers_to_mcp_servers() {
    ...
    // Seed the wrong mcp.servers style (it should migrate to top-level mcp_servers).
    let seed = r#"[mcp]
  other = "keep"
  [mcp.servers]
"#;
    ...
    ai_manager_lib::sync_enabled_to_codex(&config).expect("sync codex");
    let text = fs::read_to_string(&path).expect("read config.toml");
    // Should migrate to top-level mcp_servers and drop the wrong mcp.servers table.
    assert!(
        text.contains("mcp_servers"),
...
"#,
    ...
    let config = MultiAppConfig::default(); // nothing enabled
    ai_manager_lib::sync_enabled_to_codex(&config).expect("sync codex");
...
fn sync_enabled_to_codex_returns_error_on_invalid_toml() {
    ...
    let err = ai_manager_lib::sync_enabled_to_codex(&config).expect_err("sync should fail");
    match err {
        ai_manager_lib::AppError::Toml { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "path should reference config.toml"
            );
        }
        ai_manager_lib::AppError::McpValidation(msg) => {
            assert!(
...
fn sync_single_server_to_codex_fails_closed_on_invalid_toml() {
    ...
    // config.toml with user content plus a syntax error: the sync must fail and leave the file intact.
    let broken = "model = \"gpt-5.5\"\ninvalid = [\n";
    fs::write(&path, broken).expect("write invalid config");

    let config = MultiAppConfig::default();
    let err = ai_manager_lib::sync_single_server_to_codex(
        &config,
        "srv",
...
"#;
    ...
    ai_manager_lib::write_codex_live_atomic(&auth, Some(config_text))
    ...
    let stored_auth: serde_json::Value =
        ai_manager_lib::read_json_file(&auth_path).expect("read auth");
...
"#;
    ...
    let err = ai_manager_lib::write_codex_live_atomic(&auth, Some(config_text))
        .expect_err("config write should fail when target is directory");
    match err {
        ai_manager_lib::AppError::Io { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "io error path should point to config.toml"
            );
        }
        ai_manager_lib::AppError::IoContext { context, .. } => {
            assert!(
...
"#,
    ...
    let mut config = MultiAppConfig::default();
    let changed = ai_manager_lib::import_from_codex(&mut config).expect("import codex");
    assert!(changed >= 2, "should import both servers");

    // v3.7.0: inspect the unified structure.
    let servers = config
        .mcp
        .servers
        .as_ref()
...
"#,
    ...
    let mut config = MultiAppConfig::default();
    // v3.7.0: create a pre-existing server inside the unified structure.
    config.mcp.servers = Some(std::collections::HashMap::new());
    config.mcp.servers.as_mut().unwrap().insert(
        "existing".to_string(),
        ai_manager_lib::McpServer {
    ...
    // Verify the Codex app is enabled.
    assert!(entry.apps.codex, "Codex app should be enabled after import");

    // Verify the existing config is preserved (the server spec must not be overwritten).
...
fn sync_claude_enabled_mcp_projects_to_user_config() {
    ...
    ai_manager_lib::sync_enabled_to_claude(&config).expect("sync Claude MCP");

    let claude_path = ai_manager_lib::get_claude_mcp_path();
...
fn import_from_claude_merges_into_config() {
    ...
    config.mcp.servers.as_mut().unwrap().insert(
        ...
        ai_manager_lib::McpServer {
            ...
            apps: ai_manager_lib::McpApps {
                claude: false, // disabled initially
                claude_desktop: false,
                codex: false,
                gemini: false,
                grokbuild: false,
    ...
    let changed = ai_manager_lib::import_from_claude(&mut config).expect("import from claude");
    ...
    // Verify the Claude app is enabled.
    assert!(
        entry.apps.claude,
...
fn create_backup_skips_missing_file() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let config_path = product_data_dir().join("config.json");

    // With no file created it should return an empty string rather than error.
...
fn create_backup_generates_snapshot_file() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let config_dir = product_data_dir();
...
fn create_backup_retains_only_latest_entries() {
    ...
    // Further confirm the kept entries still include some older files, showing the cleanup only trims the excess.
...
fn sync_gemini_packycode_sets_security_selected_type() {
    ...
    // security field is written to ~/.gemini/settings.json, not product settings.json
...
fn export_sql_returns_error_for_invalid_path() {
    ...
    match err {
        AppError::IoContext { context, .. } => {
            assert!(
                context.contains(invalid_prefix.as_ref()),
                "expected IO error to mention {invalid_prefix}, got: {context}"
            );
        }
        AppError::Io { path, .. } => {
            assert!(
