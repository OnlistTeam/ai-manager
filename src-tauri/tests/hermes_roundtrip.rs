...
use ai_manager_lib::{hermes_config, update_settings, AppSettings};

/// Reading and writing back a Hermes provider must round-trip fields added in
/// Hermes v12+ or introduced later (e.g. `rate_limit_delay`, `key_env`); the UI
/// not knowing about a field is no reason to drop it silently. Otherwise advanced
/// fields configured in the Hermes web UI vanish after an edit in CC Switch.
