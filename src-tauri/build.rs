fn main() {
    println!("cargo:rerun-if-env-changed=AI_MANAGER_UPDATE_CHANNEL_ENABLED");
    println!("cargo:rerun-if-env-changed=AI_MANAGER_UPDATE_CHANNEL");
    if std::env::var("AI_MANAGER_UPDATE_CHANNEL_ENABLED").as_deref() == Ok("1") {
        match std::env::var("AI_MANAGER_UPDATE_CHANNEL").as_deref() {
            Ok("stable" | "staging") => {}
            _ => panic!("release updates require AI_MANAGER_UPDATE_CHANNEL=stable or staging"),
        }
    }
