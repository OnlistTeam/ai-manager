...
/// only), and Qwen3-Coder models (Alibaba Bailian marks built-in tools unsupported
/// for the coder series). Deliberately NOT listed by host: Volcano Ark Doubao, general
/// Alibaba Bailian Qwen models that support built-in web_search, and GPT-native relays.
    ...
    pub(crate) fn restore_preserving_newer_same_account_auth(&self) -> Result<(), AppError> {
        ...
        if failures.is_empty() {
            ...
            Err(AppError::Message(format!(
                "Failed to restore the Codex live state: {}",
                failures.join("; ")
            )))
        }
    }
...
/// Return the Codex config directory path
...
/// Return the Codex auth.json path
...
fn get_codex_managed_oauth_live_auth_marker_path() -> PathBuf {
    crate::infrastructure::paths::product_data_dir()
        .join(CODEX_MANAGED_OAUTH_LIVE_AUTH_MARKER_FILENAME)
}
...
/// Extract `account_id` from a live/backup Codex `auth`, used for recording and
/// comparing markers.
///
/// Only ChatGPT login shapes are accepted (`auth_mode == "chatgpt"`, `OPENAI_API_KEY`
/// may be empty). The full bundle written by managed accounts additionally carries
/// `tokens.refresh_token` and a top-level `last_refresh`; both are tolerated here.
/// Ownership is decided from account-scoped content: the Codex CLI refreshes itself and
/// rotates access_token, so a short-lived token fingerprint is not a stable delete
/// predicate.
...
/// Clean up the ChatGPT login a managed provider left behind in
/// `~/.codex/auth.json` when switching away from it or deleting the account from the
/// auth center.
///
/// The delete predicate is content-based on `auth_mode + tokens.account_id` rather than
/// an access-token fingerprint, which the Codex CLI's self-refresh would invalidate. A
/// native `codex login` for the same account therefore counts as that account's login;
/// the switch path must first adopt the rotated refresh token from disk back into the
/// manager before calling this function, so this account-scoped trade-off never loses
/// credentials. An explicit delete/logout in the auth center removes it on purpose.
...
pub fn ensure_codex_live_auth_unchanged_for_managed_account(
    ...
    if !auth_path.exists() {
        return Err(AppError::Message(format!(
            "The live auth of Codex CLI account {account_id} was removed during the switch; please retry"
        )));
    }
    ...
    {
        return Err(AppError::Message(format!(
            "The live credentials of Codex CLI account {account_id} were refreshed during the switch; the operation was cancelled to avoid overwriting the new refresh token, please retry"
        )));
    }
    Ok(())
}
        ...
        if codex_live_auth_is_managed_chatgpt_login(&auth, account_id) {
            if let Some(expected_refresh_token) = expected_refresh_token {
                ...
                if current_refresh_token != Some(expected_refresh_token.trim()) {
                    return Err(AppError::Message(format!(
                        "The live credentials of Codex CLI account {account_id} were refreshed during the switch; the operation was cancelled to avoid deleting the new refresh token, please retry"
                    )));
                }
            }
...
/// Decide whether the given Codex `auth` (from live auth.json or a live backup) is a
/// "managed ChatGPT login belonging to `account_id`".
///
/// Managed accounts write a **full refreshable bundle** with the same shape as a native
/// browser login (both carry refresh_token), and the Codex CLI rotates tokens so old
/// access_token fingerprints stop matching — shape/hash can no longer tell them apart.
/// So the decision is **content-based on account_id**: chatgpt mode plus a
/// `tokens.account_id` matching the managed account counts as that account's login. A
/// native login for the same account is treated identically (same account, no loss).
///
/// Used when stripping live backups, so a managed account's refreshable token is never
/// persisted into a backup config.
...
/// Read back the refresh_token / id_token belonging to `account_id` from the Codex CLI's
/// current `~/.codex/auth.json` (only when the login on disk is for that same account).
///
/// Used before switching back to a managed provider, to adopt the latest refresh_token
/// the CLI rotated on its own instead of overwriting a valid CLI login with a stale
/// token (the "plain `codex` runs in between repeated switches" scenario).
pub fn read_codex_live_auth_refresh_for_account(
    ...
    // Only adopt the refresh_token when the disk really holds "a ChatGPT login for this
    // account_id", so no token is picked up from a non-chatgpt or malformed auth.
    if !codex_live_auth_is_managed_chatgpt_login(&auth, account_id) {
        return None;
    }
...
/// Atomically write Codex's `auth.json` and `config.toml`, rolling back step one if step two fails
pub fn write_codex_live_atomic(
    ...
    // Read the previous contents for rollback
    ...
    // Step one: write auth.json
    write_json_file(&auth_path, auth)?;

    // Step two: write config.toml (roll back auth.json on failure)
...
/// Read `~/.codex/config.toml`, returning an empty string when it does not exist
...
/// Validate the syntax of a non-empty TOML text
...
/// Read and validate `~/.codex/config.toml`, returning its text (possibly empty)
...
/// Upper bound for reading the model catalog file (32 MiB). A catalog JSON is normally a
/// few hundred KiB; anything larger is treated as anomalous, so pointing at a huge
/// external file cannot exhaust memory.
...
pub fn read_codex_model_catalog_simplified_from_live() -> Result<Option<Value>, AppError> {
    ...
    let catalog_text = match read_limited_string(&catalog_path, MAX_CODEX_CATALOG_BYTES) {
        ...
        Err(error) => {
            log::warn!(
                "Refusing to read the out-of-bounds or oversized Codex model catalog {}: {error}",
...
/// Safely read a file into a string, erroring out when it exceeds the byte limit.
...
pub(crate) fn resolve_cc_switch_catalog_path(
    ...
    // Note (deliberate behaviour change): on Windows, legacy WSL-style Linux paths of
    // the form `/…` are also treated as absolute and therefore fail the containment
    // check below — previously such paths failed to match the generated file name, fell
    // back to file-name resolution and happened to work. This is acceptable: the next
    // provider switch writes a bare file name again and the config heals itself (see the
    // scenario note on `set_catalog_json_none_removes_cc_switch_owned_by_filename`).
    ...
    if !path_is_within(base_dir, &resolved) {
        log::warn!(
            "Codex model_catalog_json points outside the config directory: {} (allowed directory: {})",
            resolved.display(),
            base_dir.display()
        );
        return None;
    ...
    // Lexical containment is not runtime containment: a symlink inside the config
    // directory (e.g. ~/.codex/link -> /etc) lets `link/cc-switch-model-catalog.json`
    // pass the check above while the read lands outside. When the file exists,
    // canonicalize the real path, validate again, and return the canonical path to the
    // caller so later reads no longer traverse the symlink component.
    if resolved.exists() {
        ...
        // canonicalize the base as well so both sides share a prefix (Windows \\?\,
        // macOS /tmp -> /private/tmp); on failure fall back to the lexical base — comparing
        // a lexical base against a canonical path can only reject wrongly (degrading to
        // not reading), never allow wrongly.
        ...
        if !path_is_within(&canonical_base, &canonical) {
            log::warn!(
                "Codex model_catalog_json resolves through a symlink to outside the config directory: {} -> {} (allowed directory: {})",
                resolved.display(),
                canonical.display(),
    ...
    for (id, item) in model_providers.iter() {
        ...
        if !is_bedrock && table.get("aws").is_some() {
            return Err(AppError::localized(
                ...
                format!("Codex 0.149 refuses to load this config: the `aws` field is only allowed for the built-in amazon-bedrock / amazon-bedrock-runtime, [model_providers.{id}] must not carry it. Remove the field or use a built-in Bedrock id"),
            ));
        }
        if table.get("auth").is_some() {
            ...
            if let Some(conflict) = conflict {
                return Err(AppError::localized(
                    "provider.codex.config.invalid_provider_table",
                    format!("Codex 0.149 refuses to load this config: `auth` in [model_providers.{id}] cannot coexist with `{conflict}`. Remove one of them"),
                ));
            }
        }
    }
...
fn set_codex_experimental_bearer_token(config_text: &str, token: &str) -> Result<String, AppError> {
    if config_text.trim().is_empty() {
        return Err(AppError::localized(
            ...
            "The third-party Codex provider has no config.toml configuration, so the bearer token cannot be written",
        ));
    }
...
/// Unify Codex session history: rewrite the live config of an official provider so it
/// runs under the shared `custom` model_provider identifier (authentication still goes
/// through the ChatGPT login in `auth.json`), so official sessions created while the
/// switch is on share the same resume history bucket as third-party sessions.
///
/// Two cases refuse the injection and return the input unchanged:
/// - the config already has an explicit `model_provider`: a route the user specified by
///   hand is never overridden;
/// - the config already has a differently shaped `[model_providers.custom]` table:
///   setting `model_provider` would activate a table we do not recognize (possibly with
///   a third-party base_url/token, routing ChatGPT OAuth traffic to the wrong backend),
///   so the switch is better left ineffective for that config.
pub fn inject_codex_unified_session_bucket(config_text: &str) -> Result<String, AppError> {
    ...
    if existing_custom_conflicts {
        log::warn!(
            "The official Codex config already has a custom [model_providers.custom]; skipping unified session route injection to avoid activating an unknown route"
        );
        return Ok(config_text.to_string());
    }
...
/// The inverse of `inject_codex_unified_session_bucket`: strip the injected unified
/// session route out of the config text, so the write-back on switch never carries it
/// into the stored config in the database (turning the switch off and switching restores
/// everything). Only stripped when the shape matches the injected output exactly;
/// third-party templates and user-defined `custom` entries (with base_url and other
/// differing fields) are left untouched.
...
/// When the unified session switch is on, inject the shared custom route into the config
/// text of an official provider's `{ auth, config }` settings object; no change when the
/// switch is off or the provider is not official.
///
/// Shared by both persistence paths, the ordinary live write
/// (`write_codex_live_for_provider`) and the proxy takeover backup
/// (`update_live_backup_from_provider`): during a takeover live belongs to the proxy, so
/// the injection must go into the backup for the restored live to carry the unified route.
...
///
/// The SSOT for MCP servers is the mcp_servers table in the DB; `[mcp_servers]` in the
/// live `config.toml` is only a projection re-created by the MCP sync after every live
/// write. Baking it into the stored provider config during write-back would make servers
/// already deleted in the app reappear in live the next time that provider is activated,
/// and the per-entry reconcile only knows about rows present in the DB, so such orphans
...
pub fn strip_codex_mcp_servers_from_settings(settings: &mut Value) -> Result<(), AppError> {
    ...
    // Also clean up the historically wrong [mcp.servers] format (the live-side MCP sync performs the same migration)
...
///
/// When the unified session switch is on, official configs get the shared `custom` route
/// injected before being written to disk (see `inject_codex_unified_session_bucket`).
        ...
        Some(text) if !text.trim().is_empty() => {
            ...
            if carried_key.is_some() && codex_config_routes_third_party_without_token_slot(text) {
                return Err(AppError::localized(
                    ...
                    "A third-party Codex config must contain a custom model_providers entry to carry the API key (Codex does not recognize a top-level experimental_bearer_token)",
                ));
            }
                ...
                && codex_config_falls_back_to_official_auth_for_third_party(text)
            {
                return Err(AppError::localized(
                    ...
                    "This Codex config has no usable API key, while requires_openai_auth = true (or a top-level openai_base_url) makes Codex fall back to the login credentials in auth.json when calling the third-party address. Fill in an API key for the provider, or remove that fallback directive",
                ));
            }
    ...
    match field {
        "base_url" | "wire_api" => {
            ...
            if let Some(provider_key) = model_provider {
                // validate_reserved_model_provider_ids (since 0.148) rejects the whole
                // config when it contains a reserved id table such as
                // `[model_providers.openai]` ("Built-in providers cannot be overridden"),
                // and Codex simply fails to start. The upstream reserved check is
                // **case-sensitive** — variants like `OpenAI` are valid custom ids and
                // take the normal table-creation branch; the two bedrock ids are exempted
                // upstream, so override tables are legal for them.
                if provider_key == "openai" {
                    // Re-addressing the built-in openai goes through its proper
                    // mechanism, the top-level `openai_base_url`; wire_api is fixed inside
                    // the CLI and does not need to be written.
                    if field == "base_url" {
                        if trimmed.is_empty() {
                            doc.as_table_mut().remove("openai_base_url");
                        } else {
                ...
                if provider_key == "ollama" || provider_key == "lmstudio" {
                    // These two reserved ids have no equivalent top-level knob: creating
                    // a table would produce a config Codex refuses to load (the whole CLI
                    // fails to start during a takeover), so an explicit error beats
                    // silently writing a fatal config.
                    return Err(format!(
                        "Codex forbids overriding the built-in provider `{provider_key}` (since 0.148 it refuses to load the whole config), so its {field} cannot be rewritten; use a custom provider id instead"
                    ));
                }

                // Ensure [model_providers] table exists
                //
                // Use as_table_like_mut rather than as_table_mut: when the user writes the
                // config as an inline table (`model_providers = { foo = {...} }`, valid
                // TOML), as_table_mut returns None and we would fall through to the
                // top-level fallback below — writing the user's base_url at the wrong level
                // with no warning at all.
                if doc
                    .get("model_providers")
                    .is_none_or(|item| item.as_table_like().is_none())
                {
                    // When the key exists but is not a table (`model_providers = 42`), the
                    // line below replaces the value the user wrote by hand. The old code
                    // fell through to the top-level fallback and left it alone, so this
                    // normalization must leave a trace — consistent with the same handling
                    // in mcp/codex.rs, mcp/grokbuild.rs and opencode_config.rs.
                    if doc
                        .get("model_providers")
                        .is_some_and(|item| !item.is_none())
                    {
                        log::warn!("model_providers in config.toml is not a table; reset to an empty table");
                    }
                    ...
                    .and_then(toml_edit::Item::as_table_like_mut)
                {
                    ...
                    if let Some(provider_table) = model_providers
                        ...
                        .and_then(toml_edit::Item::as_table_like_mut)
                    {
                        // 0.149 already rejects non-bedrock tables with an empty or
                        // missing name at deserialization time ("provider name must not be
                        // empty", the whole config fails to load) — and this function is
                        // historically the source of name-less tables, so creating or
                        // editing a table must guarantee a non-empty name. The two reserved
                        // bedrock ids (the only reserved ids reachable in this branch) are
                        // exempted the other way: 0.149 only lets them override
                        // base_url/auth/http_headers/aws.*, and writing a name makes the
                        // built-in merge validation reject the whole config — which is
                        // exactly the path a proxy takeover changing base_url takes.
                ...
                log::warn!(
                    "[model_providers.{provider_key}] in config.toml has an unexpected structure; {field} was written as a top-level field"
                );
            }
...
mod tests {
    ...
    impl CodexLiveTestHome {
        fn new() -> Self {
            ...
            std::env::set_var("AI_MANAGER_TEST_HOME", dir.path());
...
"#;
    ...
    fn unified_session_bucket_preserves_other_keys_and_explicit_routing() {
        ...
        // An official config where the user explicitly set model_provider is never overridden
    ...
    fn unified_session_bucket_skips_conflicting_custom_table() {
        // A leftover custom table that is not in the injected shape: setting
        // model_provider would route official traffic to the third-party endpoint in that
        // table, so the injection must be refused entirely.
...
"#;
        ...
        // A custom table already in the injected shape (e.g. a repeated injection) still gets model_provider added
    ...
    fn unified_session_bucket_strip_keeps_third_party_custom_entry() {
        // Third-party templates also use the custom route, but their entries carry
        // base_url and other differing fields, so the shape is not the injected output and
...
"#;
    ...
    fn managed_chatgpt_login_matched_by_account_id_including_full_refresh_bundle() {
        // (1) Later managed writes produce a full bundle containing refresh_token; backup
        // stripping must recognize it by account_id so no refreshable token is persisted
        // into the live backup.
        ...
        // Non-chatgpt mode (API key) must not match.
...
"#;
    ...
    fn base_url_writes_into_inline_table_provider_section() {
        // An inline table is valid TOML, but as_table_mut() returns None for it. The old
        // code therefore fell into the "write a top-level field" fallback: the user's
        // base_url landed at the wrong level, Codex could not read it, and the UI gave no
        // hint.
        let input = r#"model_provider = "any"
...
"#;
    ...
    fn resolve_catalog_rejects_symlink_escaping_config_dir() {
        // Lexical containment can be bypassed by a symlink: ~/.codex/link -> an external
        // directory makes "link/cc-switch-model-catalog.json" lexically inside base while
        // the real read lands outside base. The second check after canonicalize must
        // reject it.
...
"#;
    ...
    fn resolve_catalog_accepts_real_file_inside_config_dir() {
        // A real file inside base: still accepted after the canonical check passes
        let temp = tempfile::tempdir().expect("tempdir");
        let base_dir = temp.path().join("codex");
        fs::create_dir_all(&base_dir).expect("create base");
