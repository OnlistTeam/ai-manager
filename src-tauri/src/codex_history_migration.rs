//! Bucket migration of third-party Codex session history.
//!
//! Only local `~/.codex` history data is migrated; the completion marker is written to the
//! device-level `settings.json`, and on failure no marker is written so the next launch retries
//! automatically.
...
use crate::config::{atomic_write, copy_file};
...
fn migrate_codex_state_db_provider_bucket(
    ...
    let mut conn = Connection::open(db_path)
        .map_err(|e| AppError::Database(format!("Failed to open the Codex state DB: {e}")))?;
    conn.busy_timeout(Duration::from_secs(5)).map_err(|e| {
        AppError::Database(format!(
            "Failed to set busy_timeout on the Codex state DB: {e}"
        ))
    })?;
    ...
    let matching_rows: i64 = conn
        ...
        .map_err(|e| {
            AppError::Database(format!(
                "Failed to count pending rows in the Codex state DB: {e}"
            ))
        })?;
    if matching_rows == 0 {
        return Ok(0);
    ...
    let tx = conn.transaction().map_err(|e| {
        AppError::Database(format!(
            "Failed to begin the Codex state DB migration transaction: {e}"
        ))
    })?;
    let changed = tx
        ...
        .map_err(|e| {
            AppError::Database(format!(
                "Failed to migrate the provider in the Codex state DB: {e}"
            ))
        })?;
    tx.commit().map_err(|e| {
        AppError::Database(format!(
            "Failed to commit the Codex state DB migration transaction: {e}"
        ))
    })?;
    Ok(changed)
}
...
fn backup_codex_state_db(
    ...
    let mut backup_conn = Connection::open(&backup_path).map_err(|e| {
        AppError::Database(format!("Failed to create the Codex state DB backup: {e}"))
    })?;
    let backup = Backup::new(source_conn, &mut backup_conn).map_err(|e| {
        AppError::Database(format!(
            "Failed to initialize the Codex state DB backup: {e}"
        ))
    })?;
    backup
        .run_to_completion(5, Duration::from_millis(25), None)
        .map_err(|e| {
            AppError::Database(format!("Failed to write the Codex state DB backup: {e}"))
        })?;
    Ok(())
}
...
"#
    ...
    fn config_sqlite_home_takes_precedence_over_codex_sqlite_home_env() {
        ...
        // TOML literal string (single quotes): Windows paths contain backslashes and a basic string would fail to parse.
