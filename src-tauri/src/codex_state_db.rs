...
mod tests {
    ...
    fn includes_config_sqlite_home() {
        ...
        // Use a TOML literal string (single quotes) for the path: Windows paths contain
        // backslashes, and a basic string (double quotes) would treat `\U`/`\s` etc. as
        // invalid escapes, causing a parse failure.
