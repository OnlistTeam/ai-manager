...
/// Response body read limit (128 MiB). A normal non-streaming completion response is
/// only tens to hundreds of KiB; anything larger is treated as an upstream anomaly or a
/// malicious payload and rejected outright, so the proxy process can't be exhausted by an
/// oversized response body or a compression bomb.
...
impl ProxyResponse {
    ...
    ///
    /// Every variant checks chunk-by-chunk while accumulating and aborts as soon as the
    /// limit is exceeded (dropping the stream cuts off the upstream connection), rather
    /// than collecting everything before comparing — otherwise an oversized plaintext
    /// body would still be fully buffered into memory, making the limit pointless.
    pub async fn bytes_with_limit(self, max_bytes: usize) -> Result<Bytes, ProxyError> {
        match self {
            Self::Buffered { body, .. } => {
                // The caller has already buffered the body in full and can't cut it off
                // mid-stream, so we can only compare after the fact
            ...
            response => {
                // Hyper / Reqwest / Streamed all accumulate chunk-by-chunk; error out
                // immediately once the budget is exceeded
...
mod tests {
    ...
    /// Spin up a minimal HTTP/1.1 server that responds with an all-zero body of
    /// `Content-Length: body_len`, writing it out in chunks and counting the bytes
    /// actually written (stop as soon as a write fails after the client disconnects).
    async fn spawn_fixed_body_server(
        ...
        tokio::spawn(async move {
            ...
            // Read the request headers to completion (their content doesn't matter)
    ...
    /// There's a delay between the client disconnecting and the server's write
    /// failing (the loopback buffer swallows a bit more), so wait a moment before
    /// reading the count. As long as the client really did cut off mid-stream, the
    /// server can never have written out most of the body.
    async fn assert_server_aborted_early(
        ...
        assert!(
            written < body_len / 2,
            "client should disconnect immediately once the budget is exhausted; server should not write out most of the body (actually wrote {written}/{body_len} bytes)"
        );
    }
    ...
    async fn bytes_with_limit_aborts_hyper_response_before_full_body_arrives() {
        ...
        // Build a real hyper::Response<Incoming>: manually establish an http1 client connection
        ...
        // Key assertion: if this regressed to "collect everything, then compare", the
        // server would write out the full 16 MiB
    ...
    async fn bytes_with_limit_aborts_reqwest_response_before_full_body_arrives() {
        ...
        // This is a loopback transport assertion. A developer's system-wide
        // proxy must not intercept 127.0.0.1 and turn the fixture into a 502.
