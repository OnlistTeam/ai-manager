...
//!
//! Selects and manages the proxy target provider, implementing smart failover
...
pub struct ProviderRouter {
    /// Database connection
    db: Arc<Database>,
    /// Circuit breaker manager - key format: "app_type:provider_id"
...
impl ProviderRouter {
    ...
    /// Selects the usable providers (failover aware)
    ///
    /// Returns the usable providers ordered by priority:
    /// - Failover off: returns only the current provider
    /// - Failover on: uses only the failover queue, trying entries in queue order (P1 -> P2 -> ...)
    pub async fn select_providers(&self, app_type: &str) -> Result<Vec<Provider>, AppError> {
        ...
        // Check whether auto failover is enabled for this app (read from the proxy_config table)
        let auto_failover_enabled = match self.db.get_proxy_config_for_app(app_type).await {
            Ok(config) => config.auto_failover_enabled,
            Err(e) => {
                log::error!("[{app_type}] failed to read proxy_config: {e}, failover disabled");
                false
            }
        };
                ...
                .is_some_and(|provider| !provider_supports_failover(app_type, provider))
        {
            ...
            // Failover on: try strictly in queue order (P1 -> P2 -> ...)
            let all_providers = self.db.get_all_providers(app_type)?;

            // Use the ordering returned by the DAO so it matches what the frontend shows
            ...
            // Failover off: use only the current provider and skip the circuit breaker check
        ...
        if result.is_empty() {
            if total_providers > 0 && circuit_open_count == total_providers {
                log::warn!("[{app_type}] [FO-004] every provider is tripped");
    ...
    /// Acquires the circuit breaker's admission permit before running a request
    ///
    /// - Closed: admitted directly
    /// - Open: once the timeout elapses, switch to HalfOpen and admit one probe
    /// - HalfOpen: admit probes according to the rate limit rules
    ///
    /// Note: the caller must release the HalfOpen slot via `record_result()` once the request
    /// finishes, or the provider stays unable to enter the probing state for a long time.
    ...
    /// Records the result of a provider request
    pub async fn record_result(
        ...
        // 1. Read the per-app circuit breaker config
        let failure_threshold = match self.db.get_proxy_config_for_app(app_type).await {
            Ok(app_config) => app_config.circuit_failure_threshold,
            Err(_) => 5, // default
        };

        // 2. Update the circuit breaker state
        let circuit_key = format!("{app_type}:{provider_id}");
        let breaker = self.get_or_create_circuit_breaker(&circuit_key).await;
        ...
        // 3. Update the health state in the database (using the configured threshold)
        self.db
    ...
    /// Resets the circuit breakers (manual recovery)
    ...
    /// Releases only the HalfOpen permit, leaving health stats untouched (neutral API)
    ///
    ...
    /// Updates the config of every circuit breaker (hot reload)
    ...
    /// Updates the config of the circuit breakers already created for an app (hot reload)
    ...
    async fn get_or_create_circuit_breaker(&self, key: &str) -> Arc<CircuitBreaker> {
        ...
        // If missing, take the write lock and create it
        ...
        // Read the per-app circuit breaker config
...
mod tests {
    ...
    async fn test_failover_enabled_uses_queue_order_ignoring_current() {
        ...
        // Set sort_index to control the order: b=1, a=2
        let mut provider_a =
            Provider::with_id("a".to_string(), "Provider A".to_string(), json!({}), None);
        ...
        // With failover on: select strictly in queue order (ignoring the current provider)
    ...
    async fn test_failover_enabled_uses_queue_only_even_if_current_not_in_queue() {
        ...
        // Put only b in the failover queue (the common setup where the current provider is not queued)
        db.add_to_failover_queue("claude", "b").unwrap();
    ...
    async fn test_release_permit_neutral_frees_half_open_slot() {
        ...
        // Circuit breaker config: trip after 1 failure, 0s timeout so HalfOpen starts immediately
        db.update_circuit_breaker_config(&CircuitBreakerConfig {
            failure_threshold: 1,
            timeout_seconds: 0,
            ..Default::default()
        ...
        // First request: take the HalfOpen probe slot
        let first = router.allow_provider_request("a", "claude").await;
        assert!(first.allowed);
        assert!(first.used_half_open_permit);

        // Second request must be rejected (the slot is taken)
        let second = router.allow_provider_request("a", "claude").await;
        assert!(!second.allowed);

        // Release the slot with release_permit_neutral (health stats untouched)
        router
            .release_permit_neutral("a", "claude", first.used_half_open_permit)
            .await;

        // Third request must be allowed (the slot was released)
