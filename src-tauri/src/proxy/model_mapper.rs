...
//!
//! Replaces the model name in a request according to the provider config before forwarding
...
impl ModelMapping {
    /// Extracts the model mapping from the provider config
    ...
    /// Checks whether any model mapping is configured
    ...
    /// Returns the mapped model for the given original model name
    pub fn map_model(&self, original_model: &str) -> String {
        let model_lower = original_model.to_lowercase();

        // 1. Match by model tier
        if model_lower.contains("fable") {
            if let Some(ref m) = self.fable_model {
                return m.clone();
            }
            // When no separate fable tier is configured, fall back to the opus tier, matching the
            // official Claude Code classifier's downgrade direction (fable -> opus) instead of losing the tier to default.
...
/// Applies the model mapping to the request body
///
...
pub fn apply_model_mapping(
    ...
    // Return immediately when no mapping is configured
...
/// Claude Code advertises a 1M-token context via the `[1M]` suffix; upstream APIs usually
/// reject this local capability marker, so it must be stripped before forwarding.
...
pub fn strip_one_m_suffix_for_upstream_from_body(mut body: Value) -> Value {
    ...
    if stripped != model {
        log::debug!("[ModelMapper] stripped local 1M marker: {model} -> {stripped}");
