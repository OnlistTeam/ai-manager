...
pub struct RequestLog {
    ...
    /// The model name actually used for pricing at write time (the resolved pricing_model_source).
    /// Persisted for backfill: rows missing a price must be recomputed against the baseline used
    /// at write time rather than guessed from model/request_model, which can all differ under routing takeover.
    /// Empty string for error rows (never priced).
...
impl<'a> UsageLogger<'a> {
    ...
    pub fn log_request(&self, log: &RequestLog) -> Result<(), AppError> {
            ...
            Some(_) => {
                    ...
                    Self::load_existing_semantic(&conn, &fallback)?
                {
                    ...
                    return Err(AppError::Database(format!(
                        "usage collision fallback primary key hit a SHA-256 collision: {fallback}"
                    )));
                }
                (fallback, false, true)
            }
        ...
        let affected_rows = conn
            ...
            .map_err(|e| AppError::Database(format!("failed to record request log: {e}")))?;
    ...
    /// Records a failed request (with richer context)
    ///
    /// Compared with log_error, this takes more parameters so the full request context is captured
    ...
    pub fn log_error_with_context(
        ...
        let log = RequestLog {
            ...
            model,
            request_model,
            // Error rows are never priced, so leave this empty (the backfill has_usage gate also skips all-zero rows)
    ...
    /// Resolves the effective multiplier and pricing-mode source (provider first, falling back to the global default)
    pub async fn resolve_pricing_config(
        &self,
        provider_id: &str,
        app_type: &str,
    ) -> (Decimal, String) {
        // The Claude Desktop gateway has no separate global billing config (the proxy_config CHECK only
        // allows claude/codex/gemini and the frontend exposes just those three), so the global default
        // inherits claude; provider-level meta overrides are still looked up under claude-desktop (the providers table stores them under that app_type).
        ...
        let default_multiplier_raw =
            match self.db.get_default_cost_multiplier(default_app_type).await {
                Ok(value) => value,
                Err(e) => {
                    log::warn!("[USG-003] default multiplier error (app_type={app_type}): {e}");
                    "1".to_string()
                }
            };
        let default_multiplier = match Decimal::from_str(&default_multiplier_raw) {
            ...
            Err(e) => {
                log::warn!(
                    "[USG-003] failed to parse the default multiplier (app_type={app_type}): {default_multiplier_raw} - {e}"
                );
                Decimal::from(1)
            }
        };
        ...
        let default_pricing_source_raw =
            match self.db.get_pricing_model_source(default_app_type).await {
                Ok(value) => value,
                Err(e) => {
                    log::warn!("[USG-003] default pricing mode error (app_type={app_type}): {e}");
        ...
        let cost_multiplier = match provider_multiplier {
            Some(value) => match Decimal::from_str(value) {
                ...
                Err(e) => {
                    log::warn!(
                        "[USG-003] failed to parse the provider multiplier (provider_id={provider_id}): {value} - {e}"
                    );
                    default_multiplier
                }
            },
        ...
        let pricing_model_source = match provider_pricing_source {
            ...
            Some(value) => {
                log::warn!("[USG-003] invalid pricing mode (provider_id={provider_id}): {value}");
    ...
    /// Calculates and records a request
    #[allow(clippy::too_many_arguments)]
    pub fn log_with_calculation(
        ...
        if pricing.is_none() && has_usage && !is_placeholder_pricing_model(&pricing_model) {
            log::warn!("[USG-002] model pricing not found, cost recorded as 0: {pricing_model}");
        }
