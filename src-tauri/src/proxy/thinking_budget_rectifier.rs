...
//!
//! Automatically fixes request errors in the Anthropic API caused by thinking budget constraints.
//! When the upstream API returns a budget_tokens-related error, the system automatically adjusts the budget parameters and retries.
...
/// Detect whether the thinking budget rectifier needs to trigger
///
/// Detection condition: the error message contains both `budget_tokens` and a `thinking`-related constraint
pub fn should_rectify_thinking_budget(
    ...
    // Aligned with CCH: only trigger when budget_tokens + thinking + the 1024 constraint are all present
...
/// Apply budget rectification to the request body
///
...
pub fn rectify_thinking_budget(body: &mut Value) -> BudgetRectifyResult {
    ...
    // Aligned with CCH: adaptive requests are not rewritten
    ...
    // Aligned with CCH: auto-create thinking when missing/invalid, then rectify
