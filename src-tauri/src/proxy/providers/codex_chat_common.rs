...
// Exhaust every reasoning field upstream may return, in priority order: reasoning_content > reasoning (string/object) > reasoning_details.
// It does not rely on the provider meta outputFormat declaration, so it can fall back to extraction for any Chat-compatible API.
