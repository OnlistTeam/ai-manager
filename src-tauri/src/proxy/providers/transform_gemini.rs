...
pub(crate) fn build_anthropic_usage(usage: Option<&Value>) -> Value {
    ...
    let cached_tokens = usage
        ...
        .and_then(|value| value.as_u64())
        .unwrap_or(0);
    // Gemini's promptTokenCount includes cache hits (cachedContentTokenCount), whereas Anthropic
    // semantics require input_tokens to be fresh input excluding cache, with cache_read listed
    // separately. After conversion this path bills as app_type=claude, and the calculator sets
    // input_includes_cache_read=false for claude so cache is no longer subtracted from input; we
    // must subtract it here or cached tokens get billed twice (once at full input price, once at
    // cache_read price). output still uses total-prompt (the subtraction only splits input/cache).
...
mod tests {
    ...
    fn gemini_to_anthropic_rectifies_tool_args_from_schema_hints() {
                    ...
                    "parts": [{
                        "functionCall": {
                            ...
                            "args": {
                                "name": "git-commit",
                                "parameters": {
                                    "args": ["analyze in detail and write the commit message"]
                                }
                            }
                        }
                    }]
        ...
        assert_eq!(
            ...
            "analyze in detail and write the commit message"
        );
