//! OpenAI Responses API streaming conversion module
//!
//! Converts Responses API SSE into Anthropic SSE format.
//!
//! The Responses API uses a lifecycle model built on named events:
...
//!
//! This differs entirely from the Chat Completions delta-chunk model, so it needs its own state machine.
...
/// Creates a conversion stream from Responses API SSE to Anthropic SSE
///
/// State machine tracks: message_id, current_model, has_sent_message_start, item/content index map
...
mod tests {
    ...
    #[tokio::test]
    async fn test_streaming_responses_multibyte_split_across_chunks_no_replacement_chars() {
        // Multi-byte text delta split across two TCP chunks.
        let full = concat!(
            "event: response.created\n",
        ...
        // Find "€" and split inside it
        let ni_start = bytes.windows(3).position(|w| w == "€".as_bytes()).unwrap();
        let split_point = ni_start + 2; // split after second byte of "€"
