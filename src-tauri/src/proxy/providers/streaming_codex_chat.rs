...
struct ChatToResponsesState {
    ...
    /// Tool calls dropped this turn for lacking a valid function name (see `finalize_tools`).
...
impl ChatToResponsesState {
    ...
    /// Key resolution for when upstream omits `index`.
    ///
    /// `index` is mandatory in the OpenAI Chat Completions protocol, but some third-party gateways omit it.
    /// Without it the frame structure cannot distinguish "a continuation frame of the same call's
    /// arguments" from "a new call", so a new key is allocated only when the frame is **provably a new
    /// call**: the delta carries a non-empty `id` that differs from every known call. Everything else
    /// folds into the last known key (0 when the map is empty), preserving existing behaviour: better to
    /// collapse two parallel calls into one than to explode one call's continuation into many items.
    fn resolve_tool_key_without_index(&self, tool_call: &Value) -> usize {
        ...
        // Upstream may send an explicit `index: usize::MAX` and then a new id with no index. This code
        // exists precisely to cope with malformed upstreams, so a bare `+1` is out (panic in debug,
        // wrap to 0 and clobber an existing call in release). On overflow fall back to merging into the
        // last known call, consistent with this function's "collapse rather than explode" bias.
        match last_key {
    ...
    /// Whether the final output of this turn holds at least one tool call item Codex can recognise.
    ...
    fn finalize(&mut self) -> Vec<Bytes> {
        ...
        // "status=completed but no tool call in output", and the agent loop inevitably ends
        // silently: exactly the #4341 shape of "answers once then stops, no error". Report the
        // failure honestly instead of faking success. As long as one valid tool call remains Codex
        // would have continued anyway, the condition does not hold, and behaviour is unchanged.
        //
        // Applies only to turns that should be `completed`: `finish_reason=length` (including a length
        // synthesized after an early stream disconnect) has its own legitimate termination story, where a
        // missing tool call name is the consequence of truncation rather than malformed upstream data.
        // Reporting tool_call_dropped there would attribute it wrongly, and accurate diagnostics are the
    ...
    fn finalize_tools(&mut self) -> Vec<Bytes> {
        ...
        for key in keys {
            ...
            // otherwise it masquerades as "this turn still has a tool call" and bypasses the failure check in finalize below.
            ...
            if has_bad_name {
                ...
                // Log structure only: arguments may contain user code, and the frontend log sink redacts by
                // allowlist, so a new field outside the list would not be handled; emit only the byte count.
                log::warn!(
...
mod tests {
    ...
    /// #4341: when upstream only emits malformed tool calls, dropping them leaves the turn with none,
    /// and Codex treats it as a normal completion and ends silently. That must be reported as a failure.
    #[tokio::test]
    async fn dropped_only_tool_call_emits_failed_without_completed() {
        let output = collect(vec![
            "data: {\"id\":\"chatcmpl_drop\",\"model\":\"kimi-k3\",\"choices\":[{\"delta\":{\"content\":\"Let me keep working on this file\"}}]}\n\n",
        ...
        assert!(!output.contains("event: response.completed"));
        // Text deltas already pushed to the client are unaffected; the user still sees what the model said.
        assert!(output.contains("Let me keep working on this file"));
    }

    /// With `finish_reason=length` (token truncation) a tool call often stops halfway with no name.
    /// That is truncation, not "malformed upstream data", so the attribution must be incomplete rather
    /// than tool_call_dropped, or the diagnostic itself is wrong.
    #[tokio::test]
    async fn truncated_turn_stays_incomplete_instead_of_failed() {
        let output = collect(vec![
            "data: {\"id\":\"chatcmpl_trunc\",\"model\":\"kimi-k3\",\"choices\":[{\"delta\":{\"content\":\"Let me take a look\"}}]}\n\n",
    ...
    /// A whitespace-only function name matches no published tool, so it must be treated like an empty
    /// name; otherwise it masquerades as "this turn still has a tool call" and bypasses the check.
    #[tokio::test]
    ...
    /// A pure text turn (no tool call delta ever seen) is unaffected by the check.
    #[tokio::test]
    async fn text_only_turn_still_completes() {
        let output = collect(vec![
            "data: {\"id\":\"chatcmpl_text\",\"model\":\"kimi-k3\",\"choices\":[{\"delta\":{\"content\":\"Done\"},\"finish_reason\":\"stop\"}]}\n\n",
            "data: [DONE]\n\n",
        ])
        .await;
        let events = parse_sse_events(&output);
    ...
    /// When upstream omits `index`, two calls with different ids must not collapse into one.
    #[tokio::test]
    ...
    /// When upstream omits `index`, an id-less arguments continuation frame must fold into the same
    /// call rather than being treated as a new call and exploded into several items.
    #[tokio::test]
    ...
    /// When upstream omits `index` and repeats the same id (some gateways repeat the whole header on
    /// every frame), it must not be judged a new call.
    #[tokio::test]
