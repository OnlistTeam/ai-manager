...
mod tests {
    ...
    fn preserves_utf8_boundaries_when_json_payload_spans_chunks() {
        ...
        let split_at = chunk.find("Ünïcödé").unwrap() + 1;
        ...
        assert!(output.contains("Ünïcödé Gemini"));
    ...
    fn rectifies_streamed_skill_args_from_nested_parameters() {
        ...
        assert!(output.contains("analyze in detail and write the commit message"));
