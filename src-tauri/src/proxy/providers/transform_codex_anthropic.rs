...
pub(crate) fn effort_to_thinking_budget(effort: &str) -> Option<u64> {
    // ultra is a Codex-only extended tier; clamp it to the same tier as max. Falling
    // through to None would make "pick the deepest thinking" disable extended thinking.
