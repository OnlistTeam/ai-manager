...
pub fn is_unsupported_image_error(error: &ProxyError) -> bool {
    ...
    // Self-evident wording: these phrases already assert "text only", which is a modality
    // rejection, so the error need not mention image/media. Gateways such as Volcano Ark report
    // "Model only support text input", which never contains "image" (issue #5025).
    // Such gateways often drop the third-person -s, so list both the s and no-s forms.
...
mod tests {
    ...
    fn detects_text_only_errors_without_image_mention() {
        // A real Volcano Ark error (issue #5025): no image/media wording, and the English is missing
        // the third-person -s, so neither the old mentions_image gate nor the "only supports text" hint catches it.
        let error = ProxyError::UpstreamError {
            status: 400,
            body: Some(
    ...
    fn glm_52_is_classified_text_only() {
        // issue #5025: GLM 5.2 on the Volcano Coding Plan is a text-only endpoint, and the
        // mapping chain glm-5.2[1M] normalizes to a glm-5.2 suffix.
        ...
        // Future vision variants (the Zhipu 4v/5v naming convention) must not be misread as text-only.
    ...
    fn preserves_cache_control_when_replacing_image() {
        // An image block may carry a prompt cache breakpoint; when replacing it with a marker,
        // cache_control must move to the new text block or the cache hit is lost.
        let mut body = json!({
            "model": "deepseek-v4-pro",
            "messages": [{
                "role": "user",
    ...
    fn heuristic_disabled_keeps_images_for_listed_text_only_models() {
        // allow_heuristic = false: the built-in list no longer strips images speculatively, so a misclassified multimodal model does not silently lose images.
        let provider = provider(json!({}));
        let mut body = json!({
    ...
    fn explicit_text_capability_replaces_even_when_heuristic_disabled() {
        // An explicit text-only declaration is declaration-driven with no false positives, so it must apply even when the heuristic is off.
        let provider = provider(json!({
            "models": [
                { "id": "deepseek-v4-pro", "input": ["text"] }
            ]
