...
/// Injects cache_control breakpoints at the key positions of the request body
pub fn inject(body: &mut Value, config: &OptimizerConfig) {
    ...
    // (c) the last non-thinking block of the last cacheable message. Tool loops usually end
    // with user/tool_result; marking only assistant would keep the latest stable prefix from hitting the cache.
    if budget > 0 {
