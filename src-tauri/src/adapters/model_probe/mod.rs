...
fn authorize(
    ...
    let builder = match protocol {
        ...
        ProviderWireProtocol::OpenAi | ProviderWireProtocol::Gemini => builder,
    };
    if api_key.is_empty() {
        return builder;
    }
...
impl AuthPlan {
    ...
    fn native(protocol: ProviderWireProtocol) -> Self {
        Self {
            ...
            fallback: match protocol {
                ...
                ProviderWireProtocol::OpenAi | ProviderWireProtocol::Gemini => None,
            },
        }
    }
    ...
    fn openai_first(protocol: ProviderWireProtocol) -> Self {
        Self {
            ...
            fallback: (protocol != ProviderWireProtocol::OpenAi).then_some(protocol),
        }
    }
}
...
async fn probe_text(
    ...
    let url = endpoint::text_url(&target.base_url, target.protocol, &request.model);
    ...
    Ok((
        Some(status.as_u16()),
        wire::text_reply(target.protocol, &parsed),
    ))
}
...
pub async fn probe_model(
    ...
    let (http_status, reply) = match request.kind {
        ProbeModelKind::Text => probe_text(client, target, request).await?,
        ProbeModelKind::Image => probe_image(client, target, request).await?,
    };
