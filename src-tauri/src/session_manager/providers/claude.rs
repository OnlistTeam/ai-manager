    ...
    fn parse_session_skips_command_caveat_and_slash_commands() {
        ...
        std::fs::write(
            ...
            concat!(
                ...
                "{\"type\":\"user\",\"message\":{\"role\":\"user\",\"content\":\"Show me the workspace changes\"},\"sessionId\":\"session-clear\",\"timestamp\":\"2026-03-06T10:01:00Z\",\"cwd\":\"/tmp/project\"}\n",
            ),
        )
        .expect("write");

        let meta = parse_session(&path).unwrap();
        assert_eq!(meta.title.as_deref(), Some("Show me the workspace changes"));
    }
