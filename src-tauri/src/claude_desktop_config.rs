...
/// Route ID prefix recognized by the Claude Desktop model menu.
pub const CLAUDE_ROUTE_PREFIX: &str = "claude-";
/// Alternative prefix (matches the frontend `ANTHROPIC_CLAUDE_ROUTE_PREFIX`).
pub const ANTHROPIC_CLAUDE_ROUTE_PREFIX: &str = "anthropic/claude-";
/// In the Claude Code env a `[1M]` suffix declares 1M context support (matched with `eq_ignore_ascii_case`).
/// The Claude Desktop schema does not accept that suffix, so the import boundary translates it into the `supports1m` field.
...
pub const DEFAULT_PROXY_ROUTES: &[ClaudeDesktopDefaultRoute] = &[
    ...
    // fable is last: when next_catalog_safe_route_id lends a legal role name to a route
    // of a non-safe brand, it still allocates in sonnet->opus->haiku order (backward
    // compatible with existing catalogs) and never lends an unrelated brand model the
    // top-tier fable name. The UI row order is controlled independently by the frontend
    // ROLE_ORDER as Sonnet/Opus/Fable/Haiku (every proxy path is reordered by
    // normalizeProxyRows) and is unrelated to the physical order here.
    ClaudeDesktopDefaultRoute {
...
pub fn is_claude_safe_model_id(model: &str) -> bool {
    ...
    // There must be a real model identifier after the role prefix; degenerate values such
    // as claude-sonnet- are rejected (otherwise they would be written into the profile and
    // make Claude Desktop fail-all and reject the whole group).
    // The fail-all validator role allowlist of Claude Desktop 1.12603.1+ now includes fable
    // (["sonnet","opus","haiku","fable","mythos"] inside app.asar), so claude-fable-* can
    // safely be written into the profile. mythos has not been released publicly, so it is
...
pub fn validate_direct_provider(provider: &Provider) -> Result<(), AppError> {
    ...
    if !provider.settings_config.is_object() {
        return Err(AppError::localized(
            ...
            "Claude Desktop direct provider config must be a JSON object",
        ));
    }
    ...
    if let Some(meta) = provider.meta.as_ref() {
        if let Some(api_format) = meta.api_format.as_deref() {
            if !api_format.trim().is_empty() && api_format != "anthropic" {
                return Err(AppError::localized(
                    ...
                    "Claude Desktop phase 1 only supports the native Anthropic Messages API",
                ));
            }
        }
        ...
        if matches!(
            ...
            return Err(AppError::localized(
                ...
                "This provider uses Claude Desktop proxy mode and cannot be written in direct mode",
            ));
        }
        ...
        if matches!(
            ...
            return Err(AppError::localized(
                ...
                "Claude Desktop direct mode does not support providers that need local proxy translation",
            ));
        }
...
pub fn validate_proxy_provider(provider: &Provider) -> Result<(), AppError> {
    ...
    if !provider.settings_config.is_object() {
        return Err(AppError::localized(
            "claude_desktop.provider.settings_not_object",
            "Claude Desktop proxy provider config must be a JSON object",
        ));
    }
        ...
        if let Some(api_format) = meta.api_format.as_deref() {
            if !matches!(
                ...
                return Err(AppError::localized(
                    "claude_desktop.provider.api_format_unsupported",
                    format!(
                        "Claude Desktop proxy mode does not support the API format: {api_format}"
                    ),
                ));
            }
        }
    ...
    if !has_proxy_base_url_and_key(provider) {
        return Err(AppError::localized(
            ...
            "Claude Desktop proxy provider is missing a base URL or API key",
        ));
    }
    ...
    for (route_id, route) in routes {
        ...
        if !upstream_model.is_empty() && upstream_model != route_id {
            return Err(AppError::localized(
                ...
                format!("Claude Desktop direct mode cannot map models: {route_id} -> {upstream_model}; use proxy mode for non-Claude official models"),
            ));
        }
...
pub fn map_proxy_request_model(mut body: Value, provider: &Provider) -> Result<Value, AppError> {
    ...
    let upstream_model = routes
        ...
        .or_else(|| {
            // Role-keyword fallback: some Claude Desktop calls (e.g. sub-agents) request the
            // full official name with a release-date suffix (claude-haiku-4-5-20251001),
            // which is not exactly equal to the short route_id exposed by the manifest
            // (claude-haiku-4-5). Classify by opus/haiku/fable/sonnet into the configured
            // route of the same tier, matching the lenient matching of the Claude Code
            // model_mapper. The local [1m] marker has already been stripped before matching;
            // the fallback still only applies to model names Claude Desktop considers safe,
            // so non-Claude routes are not mismapped.
            ...
            routes
                ...
                // When an existing user only configured the Sonnet/Opus/Haiku tiers, a fable
                // request degrades to the opus tier, matching the degradation direction of
                // the official safety classifier and avoiding a hard route_unknown error.
                // Once the user explicitly configures a fable tier, the exact role match
                // above wins.
                .or_else(|| {
                    (role == "fable")
                        .then(|| {
...
/// Classify a Claude model name / route_id by role keyword (opus / haiku / fable / sonnet).
/// Returns Some only when an unambiguous role word matches; unknown models return None (no
/// fallback, preserving the precise error semantics).
/// Same order as the frontend `routeRoleFromId` (opus -> haiku -> fable -> sonnet).
...
pub fn proxy_gateway_base_url_from_db(db: &Database) -> Result<String, AppError> {
    ...
    if config.listen_port == 0 {
        return Err(AppError::Config(
            "The Claude Desktop proxy address needs a real listening port; start the local proxy first or use a fixed port".to_string(),
        ));
    }
    Ok(format!(
        "{}{}",
...
mod tests {
    ...
    fn proxy_gateway_base_url_rejects_unresolved_ephemeral_port() {
        ...
        assert!(
            err.to_string().contains("real listening port"),
    ...
    fn claude_desktop_direct_rejects_model_mapping_to_non_claude_upstream() {
        ...
        assert!(err.to_string().contains("use proxy mode"));
    }
    ...
    fn claude_desktop_proxy_maps_dated_role_alias_via_keyword() {
        // Reproduces the report: a Claude Desktop sub-agent requests the full official name
        // with a release-date suffix (claude-haiku-4-5-20251001), which is not exactly equal
        // to the manifest's short route_id (claude-haiku-4-5), and the old logic reported
        // route_unknown. The role-keyword fallback should map it to the Haiku tier.
        ...
        // A model without any role keyword still errors, so nothing is mismapped.
    ...
    fn claude_desktop_proxy_maps_fable_to_opus_tier() {
        // issues #4026/#4049: when an existing user configured only the Sonnet/Opus/Haiku
        // tiers and no explicit fable tier, a fable request falls back to the opus tier in
        // the degradation direction of the official classifier.
        let mut provider = proxy_provider("proxy");
        provider
            .meta
            .as_mut()
        ...
        // The form with a [1m] marker and a date suffix must hit the same fallback.
    ...
    fn claude_desktop_proxy_fable_without_opus_route_still_errors() {
        // With no opus tier to fall back to, keep the precise error semantics instead of silently picking another tier.
        let mut provider = proxy_provider("proxy");
        provider
            .meta
            .as_mut()
    ...
    fn claude_desktop_proxy_maps_fable_to_dedicated_route() {
        // The fail-all validation of Desktop 1.12603.1+ allows claude-fable-5, so the user
        // can configure a separate fable tier; a fable request then hits it exactly instead
        // of degrading to opus.
        let mut provider = proxy_provider("proxy");
        provider
            .meta
            .as_mut()
        ...
        // An exact match hits the fable tier first
        let mapped = map_proxy_request_model(
            json!({"model": "claude-fable-5", "messages": []}),
            &provider,
        )
        ...
        // With a date suffix the role-keyword fallback still lands on the fable tier rather than degrading to opus
        let mapped_dated = map_proxy_request_model(
            json!({"model": "claude-fable-5-20260609", "messages": []}),
            &provider,
        )
    ...
    fn claude_desktop_rejects_1m_suffix_as_model_id() {
        ...
        assert!(!is_claude_safe_model_id("sonnet-"));
        // Degenerate values with no real identifier after the role prefix must be rejected
