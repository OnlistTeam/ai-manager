//! Hermes Agent config file read/write module
//!
//! Handles read/write operations on the `~/.hermes/config.yaml` config file (YAML format).
//! Hermes uses additive provider management: all provider configs coexist in the same file.
//!
...
/// Gets the Hermes config directory.
///
/// Resolution order mirrors Hermes's own `get_hermes_home()`:
///   1. CCS setting `hermes_config_dir` (explicit override)
///   2. `HERMES_HOME` env var (non-empty after trim; used as-is, not `~`-expanded, matching Hermes's `Path(val)`)
...
/// Platform default Hermes directory (Windows): mirrors Hermes's
/// `_get_platform_default_hermes_home()` — reads the `LOCALAPPDATA` env var,
/// falling back to `~\AppData\Local` if missing/empty, then appends `hermes`.
...
/// Platform default Hermes directory (Mac/Linux): `~/.hermes`.
#[cfg(not(target_os = "windows"))]
fn default_hermes_dir() -> PathBuf {
    crate::config::get_home_dir().join(".hermes")
}

/// Computes the Windows `%LOCALAPPDATA%\hermes` path (pure function, easy to
/// unit test across platforms). Mirrors Hermes's
/// `os.environ.get("LOCALAPPDATA", "").strip()`: if empty after trim
/// (missing/empty/whitespace-only), falls back to `<home>\AppData\Local\hermes`.
...
/// Gets the Hermes config file path.
///
...
/// Reads the Hermes config file as a `serde_yaml::Value`.
///
...
fn create_hermes_backup(source: &str) -> Result<PathBuf, AppError> {
    let backup_dir = crate::infrastructure::paths::product_data_dir()
        .join("backups")
        .join("hermes");
...
mod tests {
    ...
    ///
    /// Saves and restores `AI_MANAGER_TEST_HOME` to avoid interfering with
    ...
    fn with_test_home<T>(test_fn: impl FnOnce() -> T) -> T {
        ...
        std::env::set_var("AI_MANAGER_TEST_HOME", tmp.path());
