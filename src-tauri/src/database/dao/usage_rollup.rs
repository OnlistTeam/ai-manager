...
impl Database {
    ...
    pub fn rollup_and_prune(&self, retain_days: i64) -> Result<u64, AppError> {
        ...
        // Pruning is irreversible: once detail rows are aggregated away, zero-cost rows lose
        // any chance of being repriced from pricing_model (on startup, pricing seeding runs
        // before rollup but the startup backfill runs after it; the periodic task behaves the
        // same). So make a best-effort backfill first. A failure only warns and does not block,
        // otherwise one corrupt pricing row would permanently stall log cleanup.
        // Must be called outside the SAVEPOINT: the backfill opens its own top-level transaction.
        ...
        match result {
            Ok(deleted) => {
                ...
                if deleted > 0 {
                    ...
                    // Archiving changes the table contents, so frontend stats older than 30 days
                    // may shift; notify once so the UsageDashboard refetches
    ...
    fn do_rollup_and_prune(conn: &rusqlite::Connection, cutoff: i64) -> Result<u64, AppError> {
        ...
        // The request_model dimension preserves the "client alias -> real model" mapping from
        // route takeover; the pricing_model dimension preserves the pricing basis at write time
        // (which diverges from model in request pricing mode). Both columns may be NULL on
        // detail rows (legacy/manual data), so they are normalized to ''.
...
mod tests {
    ...
    fn test_rollup_preserves_request_model_dimension() -> Result<(), AppError> {
        ...
        {
            let conn = crate::database::lock_conn!(db.conn);
            // Route-takeover rows: model is the real upstream model, request_model the client alias.
            // Two different aliases under the same model must each get their own row so the
            // mapping stays auditable after pruning.
    ...
    fn test_rollup_preserves_pricing_model_dimension() -> Result<(), AppError> {
        ...
        {
            let conn = crate::database::lock_conn!(db.conn);
            // In request pricing mode pricing_model diverges from model, so each needs its own row
            conn.execute(
                "INSERT INTO proxy_request_logs (
    ...
    fn test_rollup_backfills_costs_before_pruning() -> Result<(), AppError> {
        ...
        {
            let conn = crate::database::lock_conn!(db.conn);
            // Zero-cost row older than 30 days: its pricing_model (gpt-5.5) is priced in the seed table.
            // Pruning is irreversible, so rollup must backfill before aggregating or the cost is
            // permanently booked as 0.
            conn.execute(
                "INSERT INTO proxy_request_logs (
                    request_id, provider_id, app_type, model, request_model, pricing_model,
                    input_tokens, output_tokens, total_cost_usd,
        ...
        // gpt-5.5 input $5/M x 1M tokens, backfilled and then aggregated
        assert!(
