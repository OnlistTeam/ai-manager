...
const MCP_SERVER_SELECT: &str =
    "SELECT id, name, server_config, description, homepage, docs, tags, enabled_claude, enabled_codex, enabled_gemini, enabled_grokbuild, enabled_opencode, enabled_hermes, enabled_claude_desktop FROM mcp_servers";
...
fn row_to_mcp_server(row: &Row<'_>) -> rusqlite::Result<(String, McpServer)> {
    ...
    let enabled_claude_desktop: bool = row.get(13)?;
...
mod tests {
    ...
    #[test]
    fn claude_desktop_has_an_independent_flag_while_unsupported_apps_stay_noop() {
        let db = Database::memory().expect("create memory db");
        let original = test_server();
        db.save_mcp_server(&original).expect("seed server");

        let desktop = db
            .update_mcp_server_app_enabled("shared-server", &AppType::ClaudeDesktop, true)
            .expect("toggle Claude Desktop")
            .expect("server exists");
        assert!(desktop.apps.claude_desktop);
        assert!(desktop.apps.gemini);
        ...
        for app in [AppType::OpenClaw, AppType::Pi] {
            let returned = db
                ...
                .expect("server exists");
            assert_eq!(returned.apps, desktop.apps);
        }
    }
}
