...
impl Database {
    ...
    /// Atomically replace the alternate-route set owned by one provider.
    ///
    /// Ordinary provider updates intentionally preserve this table because
    /// several legacy callers only load the JSON row. The product save facade
    /// calls this method only when its typed endpoint draft is present.
    pub fn replace_custom_endpoints(
        &self,
        app_type: &str,
        provider_id: &str,
        ...
        let mut entries = endpoints.iter().collect::<Vec<_>>();
        entries.sort_by(|left, right| left.0.cmp(right.0));
        for (url, endpoint) in entries {
            tx.execute(
                "INSERT INTO provider_endpoints (provider_id, app_type, url, added_at)
                 VALUES (?1, ?2, ?3, ?4)",
    ...
    /// Report whether the providers table is empty (across all app types).
    ///
    /// Used to tell a fresh install from an upgrading user; called before startup import/seed.
    /// Uses a short-circuiting `EXISTS` query, cheaper than `COUNT(*)` as the table grows.
    ...
    /// Get only the set of provider ids for the given app.
    ///
    /// Much lighter than `get_all_providers`: reads only the id column, no endpoint subquery.
    /// For cases that only need existence checks (e.g. dedup during additive-mode live sync).
    ...
    /// Report whether any provider already exists for the given app.
    ///
    /// The startup live import needs this stricter check: as soon as the app has any provider
    /// (including official seeds), `default` must not be auto-imported anymore.
    ...
    /// Report whether the given app has any provider that is not an official seed.
    ///
    /// Much lighter than `get_all_providers`: reads only the id column, no endpoint subquery,
    /// and returns on the first hit.
    /// Used by `import_default_config` to decide whether to skip the live import.
    ...
    /// Compute the next free sort_index for the given app (appending at the end).
    ...
    /// Called at startup: fill in missing official preset providers (Claude / Codex / Gemini).
    ///
    /// The `official_providers_seeded` settings flag keeps this to once per database:
    /// - Fresh user: seeds the three official presets
    /// - Upgrading user: also runs once (the flag is absent), appending at the end without
    ///   disturbing the existing order
    /// - After the user deletes a seed: it is not recreated (the flag is already true),
    ///   respecting the user's intent
    ///
    /// Combined with the UPSERT semantics of `Database::save_provider`, an accidental repeat
    /// call still will not overwrite the user's active provider (is_current is preserved).
    pub fn init_default_official_providers(&self) -> Result<usize, AppError> {
        ...
        for seed in OFFICIAL_SEEDS {
            ...
            // Skip if the id already exists (edge case: the user once used the same id manually)
        ...
        // Set the flag even when inserted == 0 (e.g. the user created the same id by hand) to avoid rechecking
        self.set_setting("official_providers_seeded", "true")?;

        Ok(inserted)
    }

    /// Fallback insert of a single official seed by id (only when that id is absent).
    ///
    /// Differences from `init_default_official_providers`:
    /// - Does not touch the global `official_providers_seeded` flag; this is an on-demand repair
    /// - Handles a single seed, with the caller choosing the id + app_type
    /// - If it already exists, the user's customization is respected and not overwritten
    ///
    /// Returns Ok(true) when a new row was inserted, Ok(false) when it already existed.
...
mod ensure_official_seed_tests {
    ...
    fn ensure_recreates_grokbuild_official_seed_after_deletion() {
        ...
        assert_eq!(provider.category.as_deref(), Some("official"));
        // Empty config: no custom model table is injected on switch, so Grok CLI falls back to its built-in OAuth login
