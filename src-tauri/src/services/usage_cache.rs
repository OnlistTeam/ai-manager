//! Usage cache for tray display (in-process, write-through).
//!
//! Written on successful usage query commands; read when the system tray
//! rebuilds its menu. Not persisted — cleared on process restart and
//! repopulated by the next automatic query or tray-hover-triggered refresh.
...
impl UsageCache {
    ...
    /// Exposes the subscription snapshot by reference, avoiding a deep copy of the
    /// whole `SubscriptionQuota` every time the tray rebuilds.
    ...
    /// Exposes script-based usage results by reference, same rationale as above.
    ...
    pub fn invalidate_script(&self, app_type: &AppType, provider_id: &str) {
        // Hot path: called once per disabled-script provider whenever the tray rebuilds.
        // Take the read lock's `contains_key` first to fast-path the common "wasn't
        // cached anyway" case, avoiding an unnecessary write-lock upgrade.
