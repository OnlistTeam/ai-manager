...
/// Single source of truth: the write side (proxy logger/calculator), the
/// backfill side (usage_stats cost recalculation), and the display side
/// (this file's SQL normalization) must all reference this list, to keep
/// the same semantics from scattering across the codebase and being missed
/// when a new app is added (grokbuild was once missed on the backfill side).
/// The frontend's `src/types/usage.ts` constant of the same name is the
/// cross-language counterpart; keep changes in sync.
pub(crate) const CACHE_INCLUSIVE_APP_TYPES: &[&str] = &["codex", "gemini", "grokbuild"];

/// Whether `app_type`'s stored `input_tokens` already includes cache read/write.
