...
/// Safely obtain the current Unix timestamp
...
fn project_prompt_set_to_path(
    ...
    let ids = enabled
        ...
        .join(", ");
    Ok(Some(format!(
        "Multiple prompts are enabled at once; the first one in stable order was projected; enabled IDs: {ids}"
    )))
}
...
impl PromptService {
    ...
    pub fn upsert_prompt(
        ...
        // Check whether this prompt is enabled
        let is_enabled = prompt.enabled;

        state.db.save_prompt(app.as_str(), &prompt)?;

        if is_enabled {
            // Enabling a prompt: write the content to the file
            let target_path = prompt_file_path(&app)?;
            write_text_file(&target_path, &prompt.content)?;
        } else {
            // Disabling a prompt: check whether any other prompt is still enabled
            let prompts = state.db.get_prompts(app.as_str())?;
            let any_enabled = prompts.values().any(|p| p.enabled);

            if !any_enabled {
                // All prompts are disabled, clear the file
                let target_path = prompt_file_path(&app)?;
    ...
    pub fn enable_prompt(state: &AppState, app: AppType, id: &str) -> Result<(), AppError> {
        ...
        // Backfill the current live file content into the enabled prompt, or create a backup
        let target_path = prompt_file_path(&app)?;
        if target_path.exists() {
            if let Ok(live_content) = std::fs::read_to_string(&target_path) {
                if !live_content.trim().is_empty() {
                    ...
                    // Try to backfill into the currently enabled prompt
                        ...
                        .map(|(id, p)| (id.clone(), p))
                    {
                        ...
                        log::info!(
                            "Backfilled live prompt content into enabled item: {enabled_id}"
                        );
                        state.db.save_prompt(app.as_str(), enabled_prompt)?;
                    } else {
                        // No enabled prompt, so create a backup once (avoiding duplicates)
                        ...
                        if !content_exists {
                            ...
                            let backup_prompt = Prompt {
                                ...
                                description: Some(
                                    "Automatic backup of the original prompt".to_string(),
                                ),
                                enabled: false,
                            ...
                            log::info!(
                                "Backfilled live prompt content, created backup: {backup_id}"
                            );
        ...
        // Enable the target prompt and write it to the file
        let mut prompts = state.db.get_prompts(app.as_str())?;
        ...
        if let Some(prompt) = prompts.get_mut(id) {
            prompt.enabled = true;
            write_text_file(&target_path, &prompt.content)?; // atomic write
            state.db.save_prompt(app.as_str(), prompt)?;
        } else {
    ...
    pub fn import_from_file(state: &AppState, app: AppType) -> Result<String, AppError> {
        ...
        let prompt = Prompt {
            ...
            content,
            description: Some("Imported from an existing config file".to_string()),
            enabled: false,
            created_at: Some(timestamp),
            updated_at: Some(timestamp),
        };
    ...
    pub fn sync_all_to_live(state: &AppState) -> Result<(), AppError> {
        ...
        for app in AppType::all() {
            ...
            if let Err(error) = Self::sync_to_live(state, app.clone()) {
                log::warn!("Failed to sync prompt to {app:?}: {error}");
    ...
    /// Auto-import from an existing prompt file on first launch (if present)
    /// Returns the number of imported prompts
    pub fn import_from_file_on_first_launch(
        state: &AppState,
        app: AppType,
    ) -> Result<usize, AppError> {
        // Idempotency guard: skip if the app already has prompts
        ...
        // Read the file content. Pi and the interactive management path share the length-capped read and coordination lock.
        let content = if matches!(app, AppType::Pi) {
            match PiAgentsFileGuard::acquire().and_then(|guard| guard.read()) {
                ...
                Err(error) => {
                    log::warn!("Failed to read prompt file: {file_path:?}, error: {error}");
                    return Ok(0);
                }
            }
        } else {
    ...
    if let Some(content) = snapshot.content.as_ref() {
        ...
        if !content.trim().is_empty() && !already_saved {
            ...
            let backup = Prompt {
                ...
                description: Some("Automatic backup of the original prompt".to_string()),
                enabled: false,
                created_at: Some(timestamp),
                updated_at: Some(timestamp),
            };
