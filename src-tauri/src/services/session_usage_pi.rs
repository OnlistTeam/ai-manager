...
pub fn sync_pi_usage(db: &Database) -> Result<SessionSyncResult, AppError> {
    let files = crate::session_manager::providers::pi::session_files()
        .map_err(|error| AppError::Config(format!("Unable to discover Pi sessions: {error}")))?;
...
fn sync_pi_files(db: &Database, files: &[PathBuf]) -> SessionSyncResult {
    ...
    for file_path in files {
        match sync_single_pi_file(db, file_path) {
            ...
            Err(error) => {
                ...
                log::warn!("[PI-SYNC] failed to parse session file: {message}");
    ...
    if result.imported > 0 {
        log::info!(
            "[PI-SYNC] sync finished: imported {}, skipped {}, scanned {} files",
            result.imported,
            result.skipped,
...
fn sync_single_pi_file(db: &Database, file_path: &Path) -> Result<SessionSyncResult, AppError> {
    let metadata = fs::symlink_metadata(file_path).map_err(|error| {
        AppError::Config(format!("Unable to read Pi session file metadata: {error}"))
    })?;
    ...
    let tx = conn.unchecked_transaction().map_err(|error| {
        AppError::Database(format!(
            "Failed to start Pi usage import transaction: {error}"
        ))
    })?;
...
fn get_pi_sync_state(db: &Database, file_path: &str) -> Result<Option<PiSyncState>, AppError> {
    ...
    let row = conn
        ...
        .optional()
        .map_err(|error| {
            AppError::Database(format!("Failed to read Pi session sync state: {error}"))
        })?;
...
fn update_pi_sync_state_on_conn(
    ...
    .map_err(|error| {
        AppError::Database(format!("Failed to update Pi session sync state: {error}"))
    })?;
    Ok(())
}
...
fn pi_file_revision(
    ...
    if tail_len > 0 {
        let mut file = File::open(file_path).map_err(|error| {
            AppError::Config(format!("Unable to open Pi session file: {error}"))
        })?;
        file.seek(SeekFrom::Start(metadata.len() - tail_len))
            .and_then(|_| file.read_exact(&mut tail))
            .map_err(|error| {
                AppError::Config(format!("Unable to read Pi session file tail: {error}"))
            })?;
    }
        ...
        file.seek(SeekFrom::Start(previous.file_size - tail_len))
            ...
            .map_err(|error| {
                AppError::Config(format!(
                    "Unable to verify Pi session append boundary: {error}"
                ))
            })?;
    }
...
fn parse_pi_file(
    ...
    let file = File::open(file_path)
        .map_err(|error| AppError::Config(format!("Unable to open Pi session file: {error}")))?;
    ...
    loop {
        ...
        let read = Read::by_ref(&mut reader)
            ...
            .map_err(|error| {
                AppError::Config(format!("Unable to read Pi session file: {error}"))
            })?;
        if read == 0 {
            return Err(AppError::Config(
        ...
        if session_id.is_none() {
            if value.get("type").and_then(Value::as_str) != Some("session") {
                return Err(AppError::Config(
                    "First valid JSON entry of the Pi session is not a session header".to_string(),
                ));
            }
            session_id = value
                .get("id")
            ...
            if let Some(byte_offset) = start_at_byte.filter(|offset| *offset >= bytes_read) {
                reader.seek(SeekFrom::Start(byte_offset)).map_err(|error| {
                    AppError::Config(format!(
                        "Unable to seek to Pi session incremental boundary: {error}"
                    ))
                })?;
...
mod tests {
    ...
    fn bounds_untrusted_provider_and_model_labels_at_utf8_boundaries() {
        ...
        let oversized = "€".repeat(MAX_USAGE_LABEL_BYTES);
        let message = entry
            .get_mut("message")
    ...
    fn oversized_file_is_reported_instead_of_silently_skipped() -> Result<(), AppError> {
        ...
        assert!(result.errors[0].contains("safety limit"));
        Ok(())
    }
}
