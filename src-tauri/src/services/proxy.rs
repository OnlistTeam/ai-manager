//! Proxy service business logic layer
//!
//! Provides start/stop and configuration management for the proxy server
...
/// Placeholder used when taking over the Live config (avoids the client warning about a missing key, without leaking the real token)
const PROXY_TOKEN_PLACEHOLDER: &str = "PROXY_MANAGED";

/// "Model override" fields that must be removed from the Claude Live config in proxy takeover mode.
///
/// Reason: in takeover mode, `*_MODEL` must be written by CC Switch as a stable Claude role alias,
/// which the local proxy then maps to the current provider's real model; `*_MODEL_NAME` must be taken over too,
/// otherwise the Claude Code model menu will retain the previous provider's display name.
const CLAUDE_MODEL_OVERRIDE_ENV_KEYS: [&str; 12] = [
    "ANTHROPIC_MODEL",
    "ANTHROPIC_REASONING_MODEL", // legacy: deprecated, but may remain in old configs
    ...
    "ANTHROPIC_SMALL_FAST_MODEL", // Legacy key (deprecated): older versions used this field to distinguish small/fast models
...
// When writing for Claude Code, keep the uppercase form used in the docs; the parser is case-insensitive.
...
impl CodexAuthFileTransaction {
    fn begin(expected: &CodexAuthFileSnapshot) -> Result<Self, String> {
        ...
        match std::fs::hard_link(&path, &probe) {
            Ok(()) => {
                std::fs::remove_file(&probe).map_err(|error| {
                    format!(
                        "Failed to clean up Codex auth transaction capability probe ({}): {error}",
                        probe.display()
                    )
                })?;
            }
            ...
            Err(error) => {
                return Err(format!(
                    "The filesystem containing Codex auth doesn't support safe recovery; original credentials unchanged ({}): {error}",
                    path.display()
                ));
            }
        }
        ...
        match std::fs::rename(&path, &quarantine) {
            ...
            Err(error) => {
                return Err(format!(
                    "Failed to claim Codex auth ({}): {error}",
                    path.display()
                ));
            }
        }
        ...
        let actual = std::fs::read(&quarantine).map_err(|error| {
            format!(
                "Failed to read claimed Codex auth ({}): {error}",
                quarantine.display()
            )
        })?;
        if actual != expected_contents {
            ...
            return match restore_result {
                ...
                Err(restore_error) => Err(format!(
                    "{}; failed to restore the newer Codex auth: {restore_error}",
                    Self::changed_error()
                )),
            };
        }
    ...
    fn install(&mut self, replacement: Option<Vec<u8>>) -> Result<(), String> {
        ...
        let write_result = (|| -> Result<(), String> {
            ...
            let mut file = options.open(&temporary).map_err(|error| {
                format!(
                    "Failed to create Codex auth temp file ({}): {error}",
                    temporary.display()
                )
            })?;
            use std::io::Write;
            file.write_all(&contents)
                ...
                .map_err(|error| {
                    format!(
                        "Failed to write Codex auth temp file ({}): {error}",
                        temporary.display()
                    )
                })?;
            drop(file);
            ...
            match std::fs::hard_link(&temporary, &self.path) {
                ...
                Err(error) => Err(format!(
                    "Failed to install Codex auth ({}): {error}",
                    self.path.display()
                )),
            }
        })();
            ...
            match std::fs::rename(&self.path, &replacement_quarantine) {
                Ok(()) => {
                    let current = std::fs::read(&replacement_quarantine).map_err(|error| {
                        format!(
                            "Failed to read Codex auth pending rollback ({}): {error}",
                    ...
                    if current == installed {
                        std::fs::remove_file(&replacement_quarantine).map_err(|error| {
                            format!(
                                "Failed to delete Codex auth pending rollback ({}): {error}",
                                replacement_quarantine.display()
                            )
                        })?;
                    } else {
                ...
                Err(error) => {
                    return Err(format!(
                        "Failed to roll back Codex auth ({}): {error}",
                        self.path.display()
                    ));
                }
            }
        ...
        match std::fs::hard_link(source, destination) {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => {
                ...
                std::fs::remove_file(source).map_err(|remove_error| {
                    format!(
                        "Failed to clean up old Codex auth transaction file ({}): {remove_error}",
                        source.display()
                    )
                })?;
                Ok(())
    ...
    fn discard_quarantined(&mut self) {
        if let Some(path) = self.quarantined.take() {
            if let Err(error) = std::fs::remove_file(&path) {
                log::warn!(
                    "Failed to clean up old Codex auth transaction file ({}): {error}",
                    path.display()
                );
            }
        }
    ...
    fn changed_error() -> String {
        "Codex auth changed during recovery; this recovery has been cancelled to avoid overwriting the new credentials, please retry".to_string()
    }
}
    ...
    fn drop(&mut self) {
        if !self.finished {
            if let Err(error) = self.rollback_inner() {
                log::error!("Codex auth transaction auto-rollback failed: {error}");
            }
        }
    }
}
...
pub struct ProxyService {
    ...
    /// AppHandle, passed to ProxyServer to support UI updates during failover
...
impl ProxyService {
    ...
    fn apply_claude_takeover_fields_for_provider(
        ...
        let auth_policy = if provider.uses_managed_account_auth() {
            // Codex-family providers (including those recognized only by base_url with no
            // provider_type meta) must keep the ANTHROPIC_AUTH_TOKEN placeholder: without that key
            // Claude Code shows a login prompt (#3784). Copilot injects the AUTH_TOKEN placeholder by
            // default too: Claude Code (measured on 2.1.220) shows a "use this custom key?" dialog for
            // ANTHROPIC_API_KEY that defaults to "No (recommended)", after which the placeholder is
            // ignored and it falls into Not logged in (this is not an sk-ant-* format check - headless
            // sends the placeholder verbatim); AUTH_TOKEN is trusted directly as the gateway Bearer, no
            // dialog. The API_KEY placeholder is only kept when the provider form explicitly chose
            // ANTHROPIC_API_KEY (meta.apiKeyField), avoiding a clash with the /login managed key (#1049).
        ...
        // During Copilot/Codex takeover the live config may still be the old provider; the displayed model must follow the target provider.
    ...
    fn apply_claude_takeover_fields_with_policy(
        ...
        // Must snapshot before remove/insert: avoids reading back the takeover alias we just wrote.
    ...
    fn apply_claude_takeover_fields_with_policy_and_models(
        ...
        match auth_policy {
            ...
            ClaudeTakeoverAuthPolicy::ManagedAccount { keep_auth_token } => {
                ...
                // Inject only one auth key: having both triggers the Claude Code warning
                // "Both ANTHROPIC_AUTH_TOKEN and ANTHROPIC_API_KEY set" (#4919).
                // - Codex family keeps AUTH_TOKEN: without it Claude Code shows a login prompt (#3784).
                //   Injected unconditionally rather than "kept only if present": the hot-switch path passes
                //   provider settings (presets lack the key) and older takeovers deleted it from existing live files.
                // - Copilot defaults to AUTH_TOKEN: an API_KEY placeholder triggers the Claude Code custom
                //   key dialog (default "No (recommended)"), which lands on Not logged in; the API_KEY
                //   placeholder is used only when the form explicitly chose ANTHROPIC_API_KEY, avoiding a
                //   clash with the /login managed key (#1049).
                if keep_auth_token {
                    env.insert(
                        "ANTHROPIC_AUTH_TOKEN".to_string(),
                        json!(PROXY_TOKEN_PLACEHOLDER),
    ...
    fn build_claude_takeover_model_fields(config: &Value) -> Vec<(&'static str, String)> {
        ...
        let opus_model = Self::claude_env_string(env, "ANTHROPIC_DEFAULT_OPUS_MODEL")
            .or(default_model)
            .or(small_fast_model);
        // No stable alias is written when Fable is unconfigured; the mapping side degrades fable->opus (same as official).
    ...
    fn claude_provider_with_effective_settings(
        ...
        .map_err(|e| format!("Failed to build the effective claude config: {e}"))?;
    ...
    pub(crate) async fn sync_codex_live_from_provider_while_proxy_active_guarded(
        ...
        .map_err(|e| format!("Failed to build the effective codex config: {e}"))?
        .settings_config;
    ...
    pub async fn sync_grok_live_from_provider_while_proxy_active(
        ...
        .map_err(|e| format!("Failed to build the effective Grok Build config: {e}"))?;
        if let Some(existing_live) = existing_live.as_ref() {
            Self::preserve_toml_mcp_servers_from_existing_config(
                &mut effective_settings,
                existing_live,
    ...
    fn get_current_provider_for_app(&self, app_type: &AppType) -> Result<Option<Provider>, String> {
        let Some(current_id) = crate::settings::get_effective_current_provider(&self.db, app_type)
            .map_err(|e| format!("Failed to get the current {app_type:?} provider: {e}"))?
        else {
            return Ok(None);
        };
        ...
        self.db
            ...
            .map_err(|e| format!("Failed to read the current {app_type:?} provider: {e}"))
    }
    ...
    async fn rollback_hot_switch_preparation(
        ...
        if let Err(error) = rollback_result {
            log::error!(
                "{} failed to restore the original backup after a failed hot switch: {error}",
                app_type.as_str()
            );
        }
        ...
        if let Some(previous_live) = previous_codex_live_state {
            if let Err(error) = previous_live.restore_preserving_newer_same_account_auth() {
                log::error!(
                    "{} failed to restore the Codex live state after a failed hot switch: {error}",
                    app_type.as_str()
                );
            }
            return;
        ...
        if let Err(error) = live_result {
            log::error!(
                "{} failed to restore the original live config after a failed hot switch: {error}",
                app_type.as_str()
            );
        }
    }
    ...
    fn require_current_provider_for_app(&self, app_type: &AppType) -> Result<Provider, String> {
        self.get_current_provider_for_app(app_type)?
            .ok_or_else(|| format!("the current {app_type:?} provider does not exist, cannot take over the live config"))
    }
    ...
    /// Whether a switch / takeover operation is currently running for this app. See `SwitchLockManager::is_locked_for_app`.
    ...
    pub async fn start(&self) -> Result<ProxyServerInfo, String> {
        // 1. Set proxy_enabled = true automatically on start
        let mut global_config = self
            .db
            .get_global_proxy_config()
            .await
            .map_err(|e| format!("Failed to get the global proxy config: {e}"))?;
        ...
        if !global_config.proxy_enabled {
            ...
            self.db
                ...
                .await
                .map_err(|e| format!("Failed to update the proxy master switch: {e}"))?;
        }
        ...
        let config = self
            ...
            .get_proxy_config()
            .await
            .map_err(|e| format!("Failed to get the proxy config: {e}"))?;

        // 3. Already running: make sure the state is persisted (if needed) and return the current info
        if let Some(server) = self.server.read().await.as_ref() {
            let status = server.get_status().await;
            return Ok(ProxyServerInfo {
                address: status.address,
                port: status.port,
                // The exact first start time cannot be recovered; returning the current time is fine for the UI
        ...
        let info = server
            .start()
            .await
            .map_err(|e| format!("Failed to start the proxy server: {e}"))?;
        if let Err(e) = self
    ...
    async fn persist_ephemeral_listen_port_if_needed(
        ...
        // The port is a global field; the legacy interface cannot write back the per-app retry and timeout config.
        ...
        self.db
            ...
            .await
            .map_err(|e| format!("Failed to save the dynamic proxy port: {e}"))
    }
    ...
    pub async fn start_with_takeover(&self) -> Result<ProxyServerInfo, String> {
        // 1. Back up the live config of every app
        self.backup_live_configs().await?;

        // 2. Sync the tokens in the live config to the database (so the proxy reads the latest token)
        if let Err(e) = self.sync_live_to_providers().await {
            // The takeover config is not written yet on failure, but the backup may hold secrets, so clean up
        ...
        // Port 0 requires starting the proxy first to learn the OS-assigned port, otherwise the takeover would write :0.
        ...
        //    even a power loss/kill during takeover is then detected and recovered on the next start.
        if let Err(e) = self.db.set_live_takeover_active(true).await {
            ...
            return Err(format!("Failed to set the takeover state: {e}"));
        }

        // 4. Take over the live config of each app (write the proxy address, clear tokens)
        if let Err(e) = self.takeover_live_configs().await {
            // Takeover failed (possibly a partial write); try to restore the original config. If that fails, keep the flag and backup for recovery on the next start.
            log::error!("Failed to take over the live config, trying to restore the original: {e}");
        ...
        match self.start().await {
            ...
            Err(e) => {
                ...
                log::error!("Proxy start failed, trying to restore the original config: {e}");
                match self.restore_live_configs().await {
                    Ok(()) => {
                        let _ = self.db.set_live_takeover_active(false).await;
                        let _ = self.db.delete_all_live_backups().await;
    ...
    /// Get the takeover state of each app (whether its live config was rewritten to the local proxy)
    pub async fn get_takeover_status(&self) -> Result<ProxyTakeoverStatus, String> {
        // Read from proxy_config.enabled (preferred), falling back to the legacy live_backup detection
    ...
    /// Enable/disable live takeover for a given app
    ///
    /// - Enable: start the proxy service automatically and take over only this app's live config
    /// - Disable: restore only this app's live config; stop the proxy service if nothing else is taken over
    pub async fn set_takeover_for_app(&self, app_type: &str, enabled: bool) -> Result<(), String> {
        ...
        if enabled {
            // 1) Start the proxy service automatically when it is not running
            if !self.is_running().await {
                self.start().await?;
            }

            // 2) Already taken over -> return (idempotent); but rebuild the takeover if the backup is missing or placeholders linger
            let current_config = self
                .db
                .get_proxy_config_for_app(app_type_str)
                .await
                .map_err(|e| format!("Failed to get the {app_type_str} config: {e}"))?;
            ...
            if current_config.enabled {
                ...
                let live_matches_current_proxy = match self
                    .live_takeover_matches_current_proxy(&app)
                    .await
                {
                    Ok(value) => value,
                    Err(e) => {
                        log::warn!("Failed to detect the {app_type_str} takeover config (continuing to rebuild the takeover): {e}");
                        false
                    }
                };

                // A real takeover requires both a backup and a live pointing at the current proxy address.
                // Looking only at placeholders would treat a half takeover / stale port as reusable and leave
                // the live file on the plain provider config after enabling takeover.
                ...
                log::warn!(
                    "{app_type_str} is marked as taken over, but backup={has_backup} live_matches_current_proxy={live_matches_current_proxy}; re-taking over and repairing live"
                );
            }

            // 3) Back up the live config (strict: error out when the target app has none)
            if restore_existing_backup_before_takeover {
                ...
                // 4) Sync the live token to the database (current app only)
            ...
            let codex_live_before_takeover = if matches!(&app, AppType::Codex) {
                Some(
                    crate::codex_config::CodexLiveStateSnapshot::capture().map_err(|error| {
                        format!("Failed to capture the pre-takeover Codex state: {error}")
                    })?,
                )
            } else {
                None
            ...
            // 5) Write the takeover config (current app only)
            if let Err(e) = self.takeover_live_config_strict(&app).await {
                log::error!(
                    "{app_type_str} failed to take over the live config, trying to restore: {e}"
                );
                match self
                ...
                {
                    Ok(()) => {
                        // Only clean up the backup after a successful restore, so the only rollback source is not lost on failure
                        let _ = self.db.delete_live_backup(app_type_str).await;
                    }
                    Err(restore_err) => {
                        log::error!(
            ...
            let enable_result = async {
                ...
                self.db
                    ...
                    .await
                    .map_err(|e| format!("Failed to set the {app_type_str} enabled state: {e}"))
            }
            .await;
            if let Err(error) = enable_result {
                log::error!("{app_type_str} failed to commit the takeover state, trying to restore live: {error}");
                match self
                    .rollback_failed_takeover_activation(&app, codex_live_before_takeover.as_ref())
                    .await
                {
                    ...
                    Err(restore_error) => {
                        log::error!(
                            "{app_type_str} failed to restore the live config, keeping the backup for a later restore: {restore_error}"
                        );
                    }
                }
                return Err(error);
        ...
        // Disabling takeover: check the enabled state
        let current_config = self
            .db
            .get_proxy_config_for_app(app_type_str)
            .await
        ...
        //
        // The with_fallback variant is required: backup -> SSOT -> placeholder cleanup, three layers.
        // The plain restore_live_config_for_app silently returns Ok(()) when the backup is missing,
        // leaving the takeover placeholders (proxy address / PROXY_MANAGED token) and a broken client.
        self.restore_live_config_for_app_with_fallback_inner(&app)
            .await?;

        // 2) Delete this app's backup (avoid storing sensitive tokens long term)
        self.db
            .delete_live_backup(app_type_str)
            .await
            .map_err(|e| format!("Failed to delete the {app_type_str} live backup: {e}"))?;
        ...
        self.db
            .update_proxy_config_for_app(updated_config)
            .await
            .map_err(|e| format!("Failed to clear the {app_type_str} enabled state: {e}"))?;

        // 4) Clear this app's health state (reset the queue state when disabling the proxy)
        self.db
            .clear_provider_health_for_app(app_type_str)
            .await
            .map_err(|e| format!("Failed to clear the {app_type_str} health state: {e}"))?;
        ...
        let any_enabled = self
            ...
            .await
            .map_err(|e| format!("Failed to check the takeover state: {e}"))?;

        if !any_enabled {
            let _ = self.db.set_live_takeover_active(false).await;

            if self.is_running().await {
                // No app is under takeover any more, so the service can be stopped
                let _ = self.stop().await;
            }
        }
    ...
    /// Synchronously disable live takeover for an app (restore the config and clear the flag, without stopping the proxy service).
    ///
    /// Used by sync paths such as `ProfileService::apply`: the calling thread may have no Tokio
    /// runtime and cannot run the stop-service / task-wait Tokio IO inside `set_takeover_for_app(false)`.
    /// This only restores the live file, deletes the backup and clears the DB takeover flag, so a later
    /// `ProviderService::switch` can write the official provider config normally.
    ///
    /// The proxy service itself keeps running; once the last app also disables takeover, it stops
    /// naturally the next time the user disables the proxy or the program exits.
    pub fn disable_takeover_for_app_sync(&self, app_type: &AppType) -> Result<(), String> {
        let app_type_str = app_type.as_str();

        // 1) Restore the original live config (backup -> SSOT -> placeholder cleanup, three layers)
        futures::executor::block_on(self.restore_live_config_for_app_with_fallback_inner(app_type))
            .map_err(|e| format!("Failed to restore the {app_type_str} live config: {e}"))?;

        // 2) Delete this app's backup
        ...
        // 4) Clear this app's health state
        futures::executor::block_on(self.db.clear_provider_health_for_app(app_type_str))
            .map_err(|e| format!("Failed to clear the {app_type_str} health state: {e}"))?;

        // 5) Clear the legacy flag
        let _ = futures::executor::block_on(self.db.set_live_takeover_active(false));

        Ok(())
    }

    /// Sync the tokens in the live config to the database
    ///
    /// Called before clearing the live tokens, ensuring the provider config in the database holds the
    /// latest token so the proxy can read the correct credentials from the database.
    async fn sync_live_to_provider(&self, app_type: &AppType) -> Result<(), String> {
        let live_config = match app_type {
            ...
            _ => return Err("this app does not support the proxy feature".to_string()),
        };
            ...
            AppType::Claude => {
                let provider_id =
                    crate::settings::get_effective_current_provider(&self.db, &AppType::Claude)
                        .map_err(|e| format!("Failed to get the current Claude provider: {e}"))?;
                                ...
                                match env_obj {
                                    ...
                                    None => {
                                        ...
                                        if let Some(root) = provider.settings_config.as_object_mut()
                                        {
                                            ...
                                            log::warn!(
                                                "Claude provider settings_config has an unexpected shape (not an object), skipping the token write (provider: {provider_id})"
                                            );
                                        }
                                    }
                                }
            ...
            AppType::Codex => {
                let provider_id =
                    crate::settings::get_effective_current_provider(&self.db, &AppType::Codex)
                        .map_err(|e| format!("Failed to get the current Codex provider: {e}"))?;
                            ...
                            .filter(|s| !s.is_empty() && *s != PROXY_TOKEN_PLACEHOLDER)
                        {
                                ...
                                .and_then(|v| v.as_object_mut())
                            {
                                ...
                                if let Some(root) = provider.settings_config.as_object_mut() {
                                    ...
                                    log::warn!(
                                        "Codex provider settings_config has an unexpected shape (not an object), skipping the token write (provider: {provider_id})"
                                    );
                                }
                            }
            ...
            AppType::Gemini => {
                let provider_id =
                    crate::settings::get_effective_current_provider(&self.db, &AppType::Gemini)
                        .map_err(|e| format!("Failed to get the current Gemini provider: {e}"))?;
                            ...
                            .filter(|s| !s.is_empty() && *s != PROXY_TOKEN_PLACEHOLDER)
                        {
                                ...
                                .and_then(|v| v.as_object_mut())
                            {
                                ...
                                if let Some(root) = provider.settings_config.as_object_mut() {
                                    ...
                                    log::warn!(
                                        "Gemini provider settings_config has an unexpected shape (not an object), skipping the token write (provider: {provider_id})"
                                    );
                                }
                            }
            ...
            AppType::GrokBuild => {
                let provider_id =
                    crate::settings::get_effective_current_provider(&self.db, &AppType::GrokBuild)
                        .map_err(|e| {
                            format!("Failed to get the current Grok Build provider: {e}")
                        })?;
                                    ...
                                    .and_then(Value::as_str)
                                {
                                    let updated =
                                        crate::grok_config::update_api_key(provider_config, &token)
                                            .map_err(|e| {
                                                format!(
                                                    "Failed to update the Grok Build API key: {e}"
                                                )
                                            })?;
                                    ...
                                    self.db
                                        ...
                                        .map_err(|e| {
                                            format!("Failed to sync the Grok Build token to the database: {e}")
                                        })?;
                                }
                            }
                        }
    ...
    async fn sync_live_to_providers(&self) -> Result<(), String> {
        ...
        log::info!("Live config token sync finished");
        Ok(())
    }
    ...
    pub async fn stop(&self) -> Result<(), String> {
        if let Some(server) = self.server.write().await.take() {
            server
                .stop()
                .await
                .map_err(|e| format!("Failed to stop the proxy server: {e}"))?;
            ...
            if global_config.proxy_enabled {
                ...
                if let Err(e) = self.db.update_global_proxy_config(global_config).await {
                    log::warn!("Failed to update the proxy master switch: {e}");
                }
            }
    ...
    /// Stop the proxy server (restore the live config; used when the user disables it manually)
    ///
    /// Clears the proxy state in the settings table so the next start does not restore automatically.
    pub async fn stop_with_restore(&self) -> Result<(), String> {
        // 1. Stop the proxy server (continue with the restore even if it was not running)
        ...
        self.db
            ...
            .await
            .map_err(|e| format!("Failed to clear the takeover state: {e}"))?;

        // 4. Clear the enabled state of every app (manual stop, no automatic restore next time)
        ...
        self.db
            ...
            .await
            .map_err(|e| format!("Failed to delete the backups: {e}"))?;

        // 6. Reset the health state (so the health badge goes back to normal)
        self.db
            .clear_all_provider_health()
            .await
            .map_err(|e| format!("Failed to reset the health state: {e}"))?;

        // Note: the failover queue and switch state are kept for the next time the proxy is enabled
        log::info!("Proxy stopped, live config restored");
        Ok(())
    }

    /// Stop the proxy server (restore the live config but keep the proxy state in the settings table)
    ///
    /// Used on a normal program exit, keeping the proxy state so the next start can restore it automatically
    pub async fn stop_with_restore_keep_state(&self) -> Result<(), String> {
        ...
        // Keep the per-app enabled and failover config so the next start restores automatically.
        // live_takeover_active is deprecated, so there is no need to write it back through the legacy interface.

        // 3. Delete the backups (the live config is restored, the backups are no longer needed)
        self.db
            .delete_all_live_backups()
            .await
            .map_err(|e| format!("Failed to delete the backups: {e}"))?;

        // 4. Reset the health state
        self.db
            .clear_all_provider_health()
            .await
            .map_err(|e| format!("Failed to reset the health state: {e}"))?;
    ...
    /// Back up the live config of every app
    async fn backup_live_configs(&self) -> Result<(), String> {
        // Claude
        if let Ok(config) = self.read_claude_live() {
            // Skip a live already taken over by the proxy: never store proxy placeholders as the "original live".
            // Otherwise a later start_with_takeover in an abnormal historical state (live already holding
            // placeholders) would overwrite a perfectly good backup; a later stop would then write the proxy
            // placeholders back into live even on the backup path, stuck on 127.0.0.1:15721 forever.
            if Self::live_has_proxy_placeholder_for_app(&AppType::Claude, &config) {
                log::warn!("claude live is already taken over by the proxy, not backing it up (avoids freezing the proxy config into the backup slot); the next stop rebuilds live from SSOT");
            } else {
                let json_str = serde_json::to_string(&config)
                    .map_err(|e| format!("Failed to serialize the Claude config: {e}"))?;
                self.db
                    .save_live_backup("claude", &json_str)
                    .await
                    .map_err(|e| format!("Failed to back up the Claude config: {e}"))?;
            }
        }
        ...
        if let Ok(mut config) = self.read_codex_live() {
            if Self::live_has_proxy_placeholder_for_app(&AppType::Codex, &config) {
                log::warn!("codex live is already taken over by the proxy, not backing it up (avoids freezing the proxy config into the backup slot); the next stop rebuilds live from SSOT");
            } else {
                self.strip_current_official_codex_auth_from_backup(&mut config)?;
                let json_str = serde_json::to_string(&config)
                    .map_err(|e| format!("Failed to serialize the Codex config: {e}"))?;
                self.db
                    .save_live_backup("codex", &json_str)
                    .await
                    .map_err(|e| format!("Failed to back up the Codex config: {e}"))?;
            }
        }
        ...
        if let Ok(config) = self.read_gemini_live() {
            if Self::live_has_proxy_placeholder_for_app(&AppType::Gemini, &config) {
                log::warn!("gemini live is already taken over by the proxy, not backing it up (avoids freezing the proxy config into the backup slot); the next stop rebuilds live from SSOT");
            } else {
                let json_str = serde_json::to_string(&config)
                    .map_err(|e| format!("Failed to serialize the Gemini config: {e}"))?;
                self.db
                    .save_live_backup("gemini", &json_str)
                    .await
                    .map_err(|e| format!("Failed to back up the Gemini config: {e}"))?;
            }
        }
        ...
        if let Ok(config) = self.read_grok_live() {
            if Self::live_has_proxy_placeholder_for_app(&AppType::GrokBuild, &config) {
                log::warn!("grokbuild live is already taken over by the proxy, not backing it up; the next stop rebuilds live from SSOT");
            } else {
                let json_str = serde_json::to_string(&config)
                    .map_err(|e| format!("Failed to serialize the Grok Build config: {e}"))?;
                self.db
                    .save_live_backup("grokbuild", &json_str)
                    .await
                    .map_err(|e| format!("Failed to back up the Grok Build config: {e}"))?;
            }
        }
    ...
    /// Back up the live config of a given app (strict mode: error out when the target config is missing)
    async fn backup_live_config_strict(&self, app_type: &AppType) -> Result<(), String> {
        ...
        // Skip a live already taken over by the proxy: never store proxy placeholders as the "original live"
        // (see the comment in backup_live_configs).
        if Self::live_has_proxy_placeholder_for_app(app_type, &config) {
            log::warn!(
                "{app_type_str} live is already taken over by the proxy, not backing it up (avoids freezing the proxy config into the backup slot); the next stop rebuilds live from SSOT"
            );
            return Ok(());
        }
        ...
        let json_str = serde_json::to_string(&config)
            .map_err(|e| format!("Failed to serialize the {app_type_str} config: {e}"))?;
        self.db
            .save_live_backup(app_type_str, &json_str)
            .await
            .map_err(|e| format!("Failed to back up the {app_type_str} config: {e}"))?;

        Ok(())
    }

    /// Build the proxy address written into live (handling 0.0.0.0 / IPv6 and similar cases)
    async fn build_proxy_urls(&self) -> Result<(String, String), String> {
        ...
        // listen_address may be 0.0.0.0 (listening on every interface), but clients cannot connect to
        // 0.0.0.0; the loopback address is therefore preferred when writing back into the app configs.
        ...
        if listen_port == 0 {
            return Err("the proxy listen port is 0 but the proxy server is not running, so a takeover address cannot be generated".to_string());
        }
    ...
    /// Whether the Grok Build live has a custom model table that can be taken over.
    ///
    /// An official-mode live (Grok CLI's own OAuth login, no `[model.*]` table) has nowhere to inject
    /// placeholders: Grok CLI distinguishes official OAuth from a custom provider by "is config empty",
    /// so it cannot express "official OAuth + custom base_url". The takeover capability gate for
    /// official providers is `official_provider_supports_proxy_takeover` (opt-in per app, currently
    /// Codex only); callers should skip the takeover or error out. Usage statistics in official mode are
    /// imported from session logs by `session_usage_grokbuild` and do not depend on the proxy.
    ...
    fn apply_grok_takeover_fields(config: &mut Value, proxy_base_url: &str) -> Result<(), String> {
        let config_toml = config
            .get("config")
            .and_then(Value::as_str)
            .ok_or_else(|| "the Grok Build config is missing the config field".to_string())?;
        let updated = crate::grok_config::apply_proxy_takeover(
            config_toml,
            proxy_base_url,
            PROXY_TOKEN_PLACEHOLDER,
        )
        .map_err(|e| format!("Failed to update the Grok Build takeover config: {e}"))?;
        config["config"] = json!(updated);
        Ok(())
    }

    /// Take over the live config of each app (write the proxy address)
    ///
    /// The proxy server already distinguishes apps by API endpoint:
    /// - `/v1/messages` → Claude
    ...
    async fn takeover_live_configs(&self) -> Result<(), String> {
        ...
        // Claude: change ANTHROPIC_BASE_URL and replace the real token with a placeholder (the proxy injects the real one)
        ...
        // Gemini: change GOOGLE_GEMINI_BASE_URL and replace the real token with a placeholder (the proxy injects the real one)
        if let Ok(mut live_config) = self.read_gemini_live() {
            if let Some(env) = live_config.get_mut("env").and_then(|v| v.as_object_mut()) {
                env.insert("GOOGLE_GEMINI_BASE_URL".to_string(), json!(&proxy_url));
                // Use a placeholder to avoid the missing-key warning
        ...
        if let Ok(mut live_config) = self.read_grok_live() {
            if Self::grok_live_config_supports_takeover(&live_config) {
                ...
                log::info!(
                    "Grok Build live config taken over, proxy address: {proxy_grok_base_url}"
                );
            } else {
                log::info!("Grok Build live is in official login mode (no custom model table), skipping the proxy takeover");
            }
        }
    ...
    /// Take over the live config of a given app (strict mode: error out when the target config is missing)
    ...
    /// Take over the live config of a given app (best effort: skip when the config is missing or unreadable)
            ...
            AppType::GrokBuild => {
                if let Ok(mut live_config) = self.read_grok_live() {
                    if Self::grok_live_config_supports_takeover(&live_config) {
                        ...
                        log::info!(
                            "Grok Build live is in official login mode (no custom model table), skipping the proxy takeover"
                        );
                    }
                }
            }
    ...
    async fn reproject_takeover_live_config_if_enabled(
        ...
        let current_config = match self.db.get_proxy_config_for_app(app_type_str).await {
            ...
            Err(error) => {
                log::warn!(
                    "Failed to read the {app_type_str} takeover state, skipping the live re-projection after the proxy restart: {error}"
                );
                return Ok(false);
            }
        };
            ...
            AppType::Claude => {
                if let Ok(Some(backup)) = self.db.get_live_backup("claude").await {
                    let config: Value = serde_json::from_str(&backup.original_config)
                        .map_err(|e| format!("Failed to parse the Claude backup: {e}"))?;
                    self.write_claude_live(&config)?;
                    log::info!("Claude live config restored");
                }
            }
            AppType::Codex => {
                if let Ok(Some(backup)) = self.db.get_live_backup("codex").await {
                    let config: Value = serde_json::from_str(&backup.original_config)
                        .map_err(|e| format!("Failed to parse the Codex backup: {e}"))?;
                    self.write_codex_restore_backup(&config)?;
                    log::info!("Codex live config restored");
                }
            }
            AppType::Gemini => {
                if let Ok(Some(backup)) = self.db.get_live_backup("gemini").await {
                    let config: Value = serde_json::from_str(&backup.original_config)
                        .map_err(|e| format!("Failed to parse the Gemini backup: {e}"))?;
                    self.write_gemini_live(&config)?;
                    log::info!("Gemini live config restored");
                }
            }
            AppType::GrokBuild => {
                if let Ok(Some(backup)) = self.db.get_live_backup("grokbuild").await {
                    let config: Value = serde_json::from_str(&backup.original_config)
                        .map_err(|e| format!("Failed to parse the Grok Build backup: {e}"))?;
                    self.write_grok_live(&config)?;
                    log::info!("Grok Build live config restored");
                }
            }
            _ => {}
        }
    ...
    pub(crate) async fn restore_live_config_for_app_with_fallback_inner(
        ...
        // 1) Prefer restoring from the live backup (the only reliable source of the "original live")
        let backup = self
            .db
            .get_live_backup(app_type_str)
            .await
            .map_err(|e| format!("Failed to get the {app_type_str} live backup: {e}"))?;
        if let Some(backup) = backup {
            let config: Value = serde_json::from_str(&backup.original_config)
                .map_err(|e| format!("Failed to parse the {app_type_str} backup: {e}"))?;

            // A backup holding proxy placeholders (abnormal history: a failed stop left live in the proxy
            // state and the next takeover wrongly backed that up as the "original live") cannot be used -
            // otherwise live would stay on 127.0.0.1:15721 forever. Fall through to the SSOT rebuild below.
            if Self::live_has_proxy_placeholder_for_app(app_type, &config) {
                log::warn!(
                    "{app_type_str} backup itself already holds proxy placeholders (abnormal historical state), skipping it and rebuilding live from SSOT"
                );
            } else {
                self.write_live_config_for_app(app_type, &config)?;
                log::info!("{app_type_str} live config restored from the backup");
                return Ok(());
            }
        }

        // 2) Fallback: no backup, but live still holds takeover placeholders (abnormal exit / historical bug)
        if !self.detect_takeover_in_live_config_for_app(app_type) {
            return Ok(());
        }

        // 2.1) Prefer rebuilding live from SSOT (the current provider) - more usable than "clean the fields"
        match self.restore_live_from_ssot_for_app(app_type) {
            Ok(true) => {
                log::info!("{app_type_str} live config restored from SSOT (no-backup fallback)");
                return Ok(());
            }
            Ok(false) => {
                log::warn!(
                    "{app_type_str} live backup is missing and SSOT restore is impossible, trying to clean the takeover placeholders"
                );
            }
            Err(e) => {
                log::error!(
                    "{app_type_str} live backup is missing and the SSOT restore failed, trying to clean the takeover placeholders: {e}"
                );
            }
        }

        // 2.2) Last resort: clean the placeholders and the local proxy address as best we can, so it does not stay stuck on proxy placeholders
        self.cleanup_takeover_placeholders_in_live_for_app(app_type)?;
        log::info!("{app_type_str} live takeover placeholders cleaned (no-backup fallback)");
        Ok(())
    }
    ...
    fn write_live_config_for_app(&self, app_type: &AppType, config: &Value) -> Result<(), String> {
        match app_type {
            ...
            _ => Err("this app does not support the proxy feature".to_string()),
        }
    }
    ...
    /// When the live backup is missing, try writing live back from SSOT (the current provider) to release the placeholder takeover.
    ///
    /// Return value:
    /// - Ok(true): written back successfully
    /// - Ok(false): no current provider / the provider does not exist / the provider itself holds placeholders, so no write back
    fn restore_live_from_ssot_for_app(&self, app_type: &AppType) -> Result<bool, String> {
        ...
        let providers = self
            ...
            .map_err(|e| format!("Failed to read the {app_type:?} provider list: {e}"))?;

        let Some(provider) = providers.get(&current_id) else {
            return Ok(false);
        };

        // A provider config that itself holds takeover placeholders must not be written back (abnormal
        // history: live was wrongly imported as a provider during takeover). Writing back would only
        // freeze the placeholders into live; Ok(false) sends the caller to the placeholder cleanup.
        if Self::live_has_proxy_placeholder_for_app(app_type, &provider.settings_config) {
            log::warn!(
                "the current {app_type:?} provider config holds proxy takeover placeholders (probably imported during takeover), skipping the SSOT write back and cleaning placeholders instead"
            );
            return Ok(false);
        }
        ...
        .map_err(|e| format!("Failed to write the {app_type:?} live config: {e}"))?;
    ...
    fn cleanup_codex_takeover_placeholders_in_live(&self) -> Result<(), String> {
        ...
        if let Some(cfg_str) = config.get("config").and_then(|v| v.as_str()) {
            ...
            let updated =
                ...
                .map_err(|e| format!("Failed to clean the Codex takeover placeholders: {e}"))?;
            let updated = crate::codex_config::remove_codex_official_proxy_route(&updated)
                .map_err(|e| format!("Failed to clean the Codex official takeover route: {e}"))?;
            config["config"] = json!(updated);
        }
    ...
    fn cleanup_grok_takeover_placeholders_in_live(&self) -> Result<(), String> {
        ...
        let updated = crate::grok_config::update_api_key(config_toml, "")
            .map_err(|e| format!("Failed to clean the Grok Build takeover placeholders: {e}"))?;
        crate::config::write_text_file(&crate::grok_config::get_grok_config_path(), &updated)
            .map_err(|e| format!("Failed to write the Grok Build config: {e}"))
    }
    ...
    /// Recover from an abnormal exit (called at startup)
    ///
    ...
    pub async fn recover_from_crash(&self) -> Result<(), String> {
        ...
        self.db
            .delete_all_live_backups()
            .await
            .map_err(|e| format!("Failed to delete the backup: {e}"))?;

        log::info!("Recovered the live config after an abnormal exit");
        Ok(())
    }

    /// Detect whether the live config is in a leftover "taken over" state
    ///
    /// Used as a fallback: when the database backup is missing but the live file already holds proxy
    /// placeholders, the startup flow can use this to trigger the recovery logic.
    ...
    /// Whether the given live/backup config has already been taken over by the proxy (holds placeholders)
    ///
    /// Purpose: detect the abnormal historical state where "the backup actually stores the proxy config".
    /// When found, the backup is untrustworthy: the backup path must not write (it would freeze the proxy
    /// config into the backup slot) and the restore path must not read (it would write the placeholders
    /// back into live and stay stuck on the proxy address). Both cases must rebuild live from SSOT.
    ...
    /// Update the live backup from the provider config (used by hot switching in proxy mode)
    ///
    /// Unlike backup_live_configs(), this builds the backup from the provider's settings_config rather
    /// than reading the live file (which is already taken over by the proxy).
    ...
    pub(crate) async fn update_live_backup_from_provider_inner(
        ...
        let mut effective_settings = if matches!(app_type_enum, AppType::Codex) {
            ...
            .map_err(|e| format!("Failed to build the effective {app_type} config: {e}"))?
            .settings_config
        } else {
        ...
        if matches!(app_type_enum, AppType::Codex) {
            ...
            let existing_backup_value = self
                ...
                .await
                .map_err(|e| format!("Failed to read the existing {app_type} backup: {e}"))?
                .map(|backup| {
                    serde_json::from_str::<Value>(&backup.original_config)
                        .map_err(|e| format!("Failed to parse the existing {app_type} backup: {e}"))
                })
                .transpose()?;
            ...
            // Unified session switch: the backup is the source for restoring live when takeover is
            // released, so the shared custom route injected into the official config must be in it.
            ...
            .map_err(|e| format!("Failed to inject the unified session route: {e}"))?;
        ...
        let backup_json = match app_type_enum {
            ...
            AppType::GrokBuild => serde_json::to_string(&effective_settings)
                .map_err(|e| format!("Failed to serialize the Grok Build config: {e}"))?,
            AppType::Gemini => {
                // The Gemini takeover only modifies .env; settings.json (with mcpServers) stays as is.
        ...
        self.db
            ...
            .await
            .map_err(|e| format!("Failed to update the {app_type} backup: {e}"))?;
    ...
    pub(crate) async fn hot_switch_provider_inner(
        ...
        let provider = self
            ...
            .map_err(|e| format!("Failed to read the provider: {e}"))?
        ...
        {
            return Err("Cannot switch to an official provider during proxy takeover".to_string());
        }

        let previous_provider_id =
            crate::settings::get_effective_current_provider(&self.db, &app_type_enum)
                .map_err(|e| format!("Failed to read the current provider: {e}"))?;
        let previous_provider = previous_provider_id
            .as_deref()
            .map(|id| {
                self.db
                    .get_provider_by_id(id, app_type_enum.as_str())
                    .map_err(|error| format!("Failed to read the previous provider: {error}"))
            })
            .transpose()?
            .flatten();
        ...
        let has_backup = self
            ...
            .await
            .map_err(|e| format!("Failed to read the {app_type} backup: {e}"))?
            .is_some();
        ...
        let previous_backup = if should_sync_backup {
            self.db
                .get_live_backup(app_type_enum.as_str())
                .await
                .map_err(|e| format!("Failed to read the original {app_type} backup: {e}"))?
        } else {
            None
        };
        let previous_codex_live_state =
            if should_sync_backup && matches!(app_type_enum, AppType::Codex) {
                Some(
                    crate::codex_config::CodexLiveStateSnapshot::capture().map_err(|error| {
                        format!("Failed to capture the pre-hot-switch Codex state: {error}")
                    })?,
                )
            } else {
                None
        ...
        let prepare_result: Result<(), String> = async {
            ...
            if has_backup && !live_taken_over && matches!(app_type_enum, AppType::Codex) {
                ...
                let auth = effective_settings
                    .get("auth")
                    .ok_or_else(|| "the Codex provider is missing its auth config".to_string())?;
                ...
                .map_err(|e| format!("Failed to write the Codex config: {e}"))?;
                if target_managed_codex_account_id.is_some() {
                    crate::codex_config::record_codex_managed_oauth_live_auth(auth).map_err(
                        |error| format!("Failed to record the Codex managed auth marker: {error}"),
                    )?;
                }
            }
        ...
        if let Err(error) = crate::settings::set_current_provider(&app_type_enum, Some(provider_id))
        {
            ...
            .await;
            return Err(format!(
                "Failed to update the local current provider: {error}"
            ));
        }
            ...
            .set_current_provider(app_type_enum.as_str(), provider_id)
        {
            if let Err(rollback_error) = crate::settings::set_current_provider(
                ...
                log::error!("Failed to restore the local current provider after a failed database switch: {rollback_error}");
            }
            self.rollback_hot_switch_preparation(
                &app_type_enum,
                previous_backup.as_ref(),
            ...
            .await;
            return Err(format!("Failed to update the current provider: {error}"));
        }
    ...
    fn preserve_toml_mcp_servers_from_existing_config(
        ...
        let target_obj = target_settings
            .as_object_mut()
            .ok_or_else(|| "a TOML app backup must be a JSON object".to_string())?;
        ...
        let mut target_doc = if target_config.trim().is_empty() {
            ...
            target_config
                ...
                .map_err(|e| format!("Failed to parse the new config.toml: {e}"))?
        };
        ...
        let existing_doc = existing_config
            .parse::<toml_edit::DocumentMut>()
            .map_err(|e| format!("Failed to parse the existing config.toml backup: {e}"))?;
    ...
    fn clear_codex_auth_in_backup(
        ...
        let Some(target_obj) = target_settings.as_object_mut() else {
            return Err("the Codex backup must be a JSON object".to_string());
        };
    ...
    fn preserve_codex_auth_in_backup(
        ...
        if let Some(config_text) = target_obj.get("config").and_then(|value| value.as_str()) {
            ...
            .map_err(|e| format!("Failed to update the Codex backup config: {e}"))?;
    ...
    /// Restoring a backup into the Codex live must never overwrite an official ChatGPT login.
    ///
    /// The backup is a snapshot taken when takeover was enabled, while the live `auth.json` may have
    /// been updated by Codex (login / token self-refresh) or by a managed account switch during
    /// takeover - so whenever live holds real login credentials (not takeover placeholders) it is newer
    /// than the snapshot and must win: if the login happened during takeover, the backup holds a
    /// third-party API key (the overwrite loop of #6277), which is demoted into the config
    /// `experimental_bearer_token`; if the backup is official-shaped too, its tokens are just an old
    /// copy and are not written back either. After the backup auth slot is removed,
    /// `write_codex_live_verbatim` only writes config.toml and never touches the live login. The check
    /// uses `codex_auth_has_credential_login_material` - leftover metadata such as `last_refresh` /
    /// `tokens.account_id` does not count as a login, so neither does an sk-plus-metadata backup escape
    /// demotion nor is a metadata-only live mistaken for an official login. `auth.json` is read
    /// independently so a corrupt config.toml cannot mask a valid login. A missing auth file must also
    /// be preserved for an official provider, because it means the user logged out during takeover;
    /// when the current provider cannot be classified, the conservative backup-side strategy applies and
    /// no old credentials of unknown origin are replayed. Symmetric with
    /// `preserve_codex_auth_in_backup` (which protects the backup direction) and likewise not gated
    /// by the "keep the official login on a non-takeover switch" setting (the takeover subsystem
    /// invariant is to never clear an official login).
    fn preserve_codex_oauth_login_on_restore(
        ...
        if live_has_login {
                ...
                .map(str::to_string)
            {
                ...
                match crate::codex_config::prepare_codex_provider_live_config(
                    ...
                    Err(e) => {
                        // Keeping the login still wins when the demotion fails: an API key can be
                        // rewritten from the DB provider config at any time, while an overwritten OAuth login requires signing in again.
                        log::warn!(
                            "Codex restore: demoting the backup API key into config failed, only skipping the auth overwrite: {e}"
                        );
                    }
                }
            }
            ...
            log::info!(
                "Codex restore: live holds official ChatGPT login credentials (always newer than the backup snapshot), keeping the login and restoring only config"
            );
            return Ok(auth_snapshot);
        }
        ...
        if missing_preservable_auth {
            target_obj.remove("auth");
            log::info!(
                "Codex restore: keeping the currently missing auth state, restoring only config"
            );
        }
        Ok(auth_snapshot)
    }

    /// Switch provider in proxy mode (hot switch, refreshing the proxy-safe live display fields as needed)
    pub async fn switch_proxy_target(
        ...
        if outcome.logical_target_changed {
            ...
            log::debug!(
                "Proxy mode: {app_type} is already aligned with the target provider {provider_id}"
            );
        }
        Ok(())
    }
    ...
    /// While Codex is taken over, the local client must keep talking to the proxy with the Responses wire API.
    /// Whether the real upstream uses Chat Completions is decided by the provider config and converted inside the proxy.
    fn apply_codex_proxy_toml_config_for_provider(
        ...
        if provider.is_some_and(crate::proxy::providers::is_codex_official_provider) {
            return crate::codex_config::apply_codex_official_proxy_route(toml_str, proxy_url)
                .map_err(|e| format!("Failed to build the Codex official takeover config: {e}"));
        }

        let updated = crate::codex_config::update_codex_toml_field(toml_str, "base_url", proxy_url)
            .map_err(|e| format!("Failed to update the Codex proxy address: {e}"))?;
        let mut updated =
            crate::codex_config::update_codex_toml_field(&updated, "wire_api", "responses")
                .map_err(|e| format!("Failed to update the Codex wire_api: {e}"))?;
            ...
            provider.and_then(crate::proxy::providers::codex_provider_upstream_model)
        {
            updated =
                crate::codex_config::update_codex_toml_field(&updated, "model", &upstream_model)
                    .map_err(|e| format!("Failed to update the Codex upstream model: {e}"))?;
        }
    ...
    fn read_claude_live(&self) -> Result<Value, String> {
        ...
        let mut value: Value =
            read_json_file(&path).map_err(|e| format!("Failed to read the Claude config: {e}"))?;
        ...
        if !value.is_object() {
            ...
            return Err(format!(
                "Claude config file is malformed: the root node must be a JSON object (currently {kind}), path: {}",
                path.display()
            ));
        }
    ...
    fn write_claude_live(&self, config: &Value) -> Result<(), String> {
        ...
        write_json_file(&path, &settings)
            .map_err(|e| format!("Failed to write the Claude config: {e}"))
    }

    fn read_codex_live(&self) -> Result<Value, String> {
        crate::codex_config::read_codex_live_settings()
            .map_err(|e| format!("Failed to read the Codex live config: {e}"))
    }
    ...
    fn write_codex_live_for_provider(
        ...
        let auth = config
            .get("auth")
            .ok_or_else(|| "the Codex config is missing the auth field".to_string())?;
    ...
    fn codex_auth_file_has_login() -> bool {
        let auth = match CodexAuthFileSnapshot::capture().and_then(|snapshot| snapshot.value()) {
            ...
            Err(error) => {
                log::warn!("Codex auth.json is unreadable, treating it as not logged in: {error}");
                return false;
            }
        };
        !Self::codex_auth_has_proxy_placeholder(&auth)
        ...
        if official_passthrough || placeholder_auth {
            ...
            if managed_official {
                let auth = config.get("auth").ok_or_else(|| {
                    "the managed Codex official config is missing the auth field".to_string()
                })?;
                ...
                crate::codex_config::record_codex_managed_oauth_live_auth(auth)
                    .map_err(|e| format!("Failed to record the Codex managed auth marker: {e}"))?;
                return Ok(());
            }
            ...
            && config.get("modelCatalog").is_some()
        {
            Some(
                crate::codex_config::CodexModelCatalogFileSnapshot::capture()
                    .map_err(|e| format!("Failed to capture the Codex model catalog: {e}"))?,
            )
        } else {
            None
        };
            ...
            Err(error) => {
                if let Some(snapshot) = catalog_snapshot.as_ref() {
                    snapshot.restore().map_err(|rollback_error| {
                        format!("{error}; rolling back the Codex model catalog also failed: {rollback_error}")
                    })?;
                }
                return Err(error);
            }
            ...
            (|| -> Result<(), String> {
                let replacement =
                    if auth.as_object().is_some_and(Map::is_empty) {
                        ...
                        Some(serde_json::to_vec_pretty(auth).map_err(|error| {
                            format!("Failed to serialize the Codex auth: {error}")
                        })?)
                    };
                ...
                if let Err(error) = transaction.install(replacement) {
                    return match transaction.rollback() {
                        ...
                        Err(rollback_error) => Err(format!(
                            "{error}; rolling back the Codex auth also failed: {rollback_error}"
                        )),
                    };
                }
                ...
                let config_result = prepared_cfg.as_deref().map_or(Ok(()), |cfg| {
                    crate::config::write_text_file(&get_codex_config_path(), cfg)
                        .map_err(|error| format!("Failed to write the Codex config: {error}"))
                });
                match config_result {
                ...
                (Some(auth), None) => {
                    if auth.as_object().is_some_and(Map::is_empty) {
                        ...
                        write_json_file(&get_codex_auth_path(), auth)
                            .map_err(|e| format!("Failed to write the Codex auth: {e}"))
                    }
                }
    ...
    fn read_gemini_live(&self) -> Result<Value, String> {
        ...
        let env_map =
            read_gemini_env().map_err(|e| format!("Failed to read the Gemini env: {e}"))?;
    ...
    fn write_gemini_live(&self, config: &Value) -> Result<(), String> {
        ...
        let env_map =
            json_to_env(config).map_err(|e| format!("Failed to convert the Gemini config: {e}"))?;
        write_gemini_env_atomic(&env_map)
            .map_err(|e| format!("Failed to write the Gemini env: {e}"))?;
        Ok(())
    }

    fn read_grok_live(&self) -> Result<Value, String> {
        crate::grok_config::read_grok_live_settings()
            .map_err(|e| format!("Failed to read the Grok Build config: {e}"))
    }
    ...
    pub async fn update_config(&self, config: &ProxyConfig) -> Result<(), String> {
        // Record the old config to decide whether a restart is needed
        let previous = self
            .db
            .get_proxy_config()
            .await
        ...
        // Save to the database (leaving live_takeover_active unchanged)
        ...
        self.db
            ...
            .await
            .map_err(|e| format!("Failed to save the proxy config: {e}"))?;
        ...
        if require_restart {
            if let Some(server) = server_guard.take() {
                server.stop().await.map_err(|e| {
                    format!("Failed to stop the proxy server before restarting: {e}")
                })?;
            }
            ...
            let info = new_server
                .start()
                .await
                .map_err(|e| format!("Failed to restart the proxy server: {e}"))?;
            if let Err(e) = self
            ...
            *server_guard = Some(new_server);
            ...
            // When any app's live is taken over, the proxy address in live must be updated too (otherwise clients still point at the old port).
            // The server write lock must be released before acquiring each app's switch lock:
            // set_takeover_for_app takes switch lock -> server lock, and the reverse order deadlocks.
            drop(server_guard);
            ...
            if updated_any {
                log::info!("Updated the proxy address in the live configs");
            }
    ...
    ///
    /// When the proxy server is running, apply the new config to every existing circuit breaker instance
    ...
    /// Hot-update the circuit breaker config of a given app
    pub async fn update_circuit_breaker_config_for_app(
        ...
        if let Some(server) = self.server.read().await.as_ref() {
            server
                ...
                .await;
            log::info!("Hot-updated the running {app_type} circuit breaker config");
        } else {
            log::debug!(
                "The {app_type} circuit breaker config takes effect on the next proxy start"
            );
        }
        Ok(())
    }

    /// Reset the circuit breaker of a given provider
    ///
    /// When the proxy server is running, reset the in-memory circuit breaker state immediately
...
mod tests {
    ...
    fn managed_account_claude_takeover_sources_codex_models_from_provider() {
        ...
        // Codex family keeps only AUTH_TOKEN; both keys would trigger a Claude Code warning (#4919)
        assert_env_str(env, "ANTHROPIC_API_KEY", None);
        assert_env_str(env, "ANTHROPIC_AUTH_TOKEN", Some(PROXY_TOKEN_PLACEHOLDER));
    }
    ...
    fn managed_account_claude_takeover_codex_injects_auth_token_without_preexisting_key() {
        ...
        // Fresh install / hot switch shape: the incoming env has no token key at all.
        let mut live_config = provider.settings_config.clone();
        ProxyService::apply_claude_takeover_fields_for_provider(
            &mut live_config,
            "http://127.0.0.1:15721",
    ...
    fn managed_account_claude_takeover_codex_by_base_url_keeps_auth_token() {
        // A provider with no provider_type meta that is recognized as managed codex purely by base_url
        // must keep the AUTH_TOKEN placeholder too (same decision family as the policy selection).
    ...
    // #4919 reproduction: switching from a third-party Claude provider (live already has AUTH_TOKEN)
    // to a managed Codex provider must keep only the AUTH_TOKEN placeholder and never write API_KEY too.
    #[test]
...
"#;
    ...
    async fn switch_proxy_target_updates_live_backup_when_taken_over() {
        ...
        // Assert: the current provider in the local settings is synced
        assert_eq!(
            crate::settings::get_current_provider(&AppType::Claude).as_deref(),
            Some("b")
        );

        // Assert: the live backup now holds the target provider config (used by stop_with_restore)
        let backup = db
...
"#;
        ...
        assert!(
            message.contains("Failed to write the Codex config")
                || message.contains("Atomic replace failed")
                || (message.contains("Failed to capture the pre-hot-switch Codex state")
...
"#
    ...
    async fn restore_preserves_logged_out_official_codex_state() {
        ...
        assert!(error.contains("Failed to read Codex auth"));
        assert_eq!(
    ...
    fn guarded_codex_restore_rejects_a_changed_auth_file() {
        ...
        assert!(error.contains("changed during recovery"));
