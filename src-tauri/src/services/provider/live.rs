...
fn apply_codex_oauth_claude_context_defaults(settings: &mut Value, provider: &Provider) {
    ...
    for (key, default_value) in [
        ...
        match provider_env.and_then(|provider_env| provider_env.get(key)) {
            ...
            // The old model injects no default, and values dragged in by a legacy shared fragment are stripped
            None => {
                env.remove(key);
            }
        }
...
fn apply_codex_official_auth(
    ...
    let Some(manager) = codex_oauth_manager else {
        return Err(AppError::Message(
            "Codex OAuth managed account is unavailable, restart the app and try again".to_string(),
        ));
    };
...
/// Build the complete refreshable auth written to the managed Codex `auth.json` (refresh_token + last_refresh).
///
/// Steps:
/// 1. **Read back**: if the Codex CLI already refreshed and rotated the refresh_token, adopt the latest
///    on-disk value first so a stale refresh_token cannot overwrite the CLI's valid login (repeated switching).
/// 2. Obtain a valid token bundle (refreshing access_token when necessary).
/// 3. Build the complete auth in the shape of a native browser login.
///
/// No outer lock is held any more: the manager refreshes per account under its own lock, so a network
/// stall cannot affect other account operations or token reads.
fn get_codex_managed_oauth_live_auth_value(
    ...
    std::thread::spawn(move || {
        tauri::async_runtime::block_on(async move {
                ...
                crate::codex_config::read_codex_live_auth_refresh_for_account(&account_id)
            {
                ...
                {
                    log::warn!(
                        "Failed to read back the Codex CLI rotated refresh_token (account={account_id}): {err}"
                    );
                }
            }
            ...
            let bundle = manager
                ...
                .map_err(|err| {
                    format!(
                        "Codex OAuth account {account_id} failed to authenticate, sign in to the ChatGPT account again: {err}"
                    )
                })?;
            let id_token = bundle
                .id_token
                ...
                .ok_or_else(|| {
                    format!(
                        "Codex OAuth account {account_id} is missing id_token, sign in again in the auth center before saving"
                    )
                })?;
    ...
    .join()
    .map_err(|_| AppError::Message("Codex OAuth token fetch thread exited abnormally".to_string()))?
...
pub(crate) fn prepare_codex_managed_oauth_live_auth_switch_away(
    ...
    .map_err(|_| {
        AppError::Message(
            "Codex OAuth live credential adoption thread exited abnormally".to_string(),
        )
    })?
    .map_err(AppError::Message)
}
...
pub(crate) fn codex_managed_oauth_live_auth(
    ...
    // Aligned with the shape of a native Codex browser login: tokens field order is id_token,
    // access_token, refresh_token, account_id, plus a top-level last_refresh. refresh_token **must** be
    // present, otherwise the Codex CLI cannot self-refresh once access_token expires ("bare codex" silently breaks).
...
/// Strictly symmetric with `apply_codex_oauth_claude_context_defaults`: injected values only live in
/// the live config and must be stripped on switch-away backfill, otherwise a program default freezes
/// into the provider's "explicit user value" and later default or model changes can never override it.
/// Strip only when "the injection would happen, it injects exactly this value, and the stored config
/// had no explicit value"; explicitly stored values and hand-edited live numbers are both kept.
...
fn restore_live_settings_for_provider_backfill(
    ...
    // MCP servers are owned by the DB mcp_servers table; the [mcp_servers] in live is a synced projection.
    // Strip it on backfill, otherwise deleted servers resurrect with the provider snapshot (per-entry reconcile cannot clear orphans).
    ...
    // The shared `custom` route injected by the unified session switch belongs to the live config only;
    // it must be stripped on switch backfill, otherwise the official provider's stored config is polluted and cannot be restored once the switch is off.
...
/// Strip the live `auth` when backfilling a managed Codex official provider.
///
/// The stored config of a managed provider must **never** persist a real OAuth token: tokens are kept
/// centrally per account by `CodexOAuthManager`; the provider config only keeps the binding and a placeholder auth.
///
/// So whatever live currently holds (the managed auth we wrote, a token rotated by the CLI, or the
/// user's own native browser-login auth with real access/refresh tokens) is uniformly replaced with
/// the placeholder auth stored on the provider, so real credentials are never backfilled into the DB config.
fn strip_codex_managed_oauth_auth_for_backfill(provider: &Provider, settings: &mut Value) {
    ...
    // Nothing to do when live has no auth.
...
pub(crate) fn sync_all_providers_to_live(
    state: &AppState,
    app_type: &AppType,
) -> Result<(), AppError> {
...
pub(crate) fn sync_current_provider_for_app_to_live(
    ...
    // This function means "sync this app to live", so the MCP re-projection targets only that app;
    // a full sync_all_enabled would drag in live corruption from unrelated apps. A failed projection
    // propagates (no degradation): there is no changed DB state to protect here, the caller can retry.
...
fn proxy_owns_live_config(
    ...
    let takeover_enabled = match futures::executor::block_on(
        state.db.get_proxy_config_for_app(app_type.as_str()),
    ) {
        Ok(config) => config.enabled,
        Err(err) => {
            log::warn!(
                    "Failed to read the {} proxy takeover flag; treating it as not taken over and continuing to write the live config; \
                     if the app really is under takeover right now, this write overwrites the taken-over live: {err}",
                    app_type.as_str()
                );
            false
        }
...
pub(crate) fn sync_live_for_provider_respecting_takeover(
    ...
    let has_live_backup = match futures::executor::block_on(
        ...
        Err(err) => {
            log::warn!(
                    "Failed to read the {} live backup; treating it as absent and continuing to write the live config: {err}",
                    app_type.as_str()
                );
            false
        }
    ...
    if !proxy_owns_live_config(state, app_type, has_live_backup, live_taken_over) {
        ...
        if has_live_backup {
            if let Err(err) = futures::executor::block_on(
                ...
                log::warn!(
                    "Failed to refresh the leftover {} live backup (does not affect this live write): {err}",
                    app_type.as_str()
                );
            }
        }
    ...
    match app_type {
        ...
        .map_err(|e| AppError::Message(format!("Failed to sync the Claude live config: {e}")))?,
        AppType::Codex if live_taken_over => futures::executor::block_on(
            state
                .proxy_service
                .sync_codex_live_from_provider_while_proxy_active(provider),
        )
        .map_err(|e| AppError::Message(format!("Failed to sync the Codex live config: {e}")))?,
        ...
        .map_err(|e| {
            AppError::Message(format!("Failed to sync the Grok Build live config: {e}"))
        })?,
        _ => {}
    }
...
/// This ensures an invalid ID falls back to the database automatically after a config import.
///
...
pub fn sync_current_to_live(state: &AppState) -> Result<(), AppError> {
    ...
    for app_type in AppType::all() {
        ...
        if let Err(error) = result {
            log::warn!("Failed to sync provider to {app_type:?}: {error}");
    ...
    for app_type in AppType::all() {
        if let Err(e) = crate::services::skill::SkillService::sync_to_app(&state.db, &app_type) {
            log::warn!("Failed to sync skill to {app_type:?}: {e}");
...
pub fn import_default_config(state: &AppState, app_type: AppType) -> Result<bool, AppError> {
    ...
    // Allow importing live even when only the official seed presets exist:
    // - startup order is import first then seed, so providers is empty for a new user and import runs as usual
    // - an existing user already has a non-seed provider, so import is skipped (correct)
    // - when the user clicks import in ProviderEmptyState, it coexists with the official seed instead of being blocked
    if state.db.has_non_official_seed_provider(app_type.as_str())? {
        return Ok(false);
    }

    // Refuse to import a "proxy-taken-over live" as a provider: during takeover live only holds the
    // PROXY_MANAGED placeholder and the local proxy address, not the user's real config. Once imported
    // it becomes the current provider (SSOT), and the later "restore without backup" path writes the
    // placeholder back to live as if it were the real config, permanently stuck on a dead local proxy.
    // Typical trigger: switching app_config_dir and restarting while takeover is on, first import into the new database.
    if state
        .proxy_service
        .detect_takeover_in_live_config_for_app(&app_type)
    {
    ...
    let settings_config = match app_type {
        ...
        AppType::GrokBuild => {
            ...
            let config = settings
                ...
                .and_then(Value::as_str)
                .unwrap_or_default();
            // The official login state (no custom model table) must error here: this function is also
            // called by startup auto-import, and the project-wide convention is "startup auto-import only
            // produces default, never an official entry" - otherwise a deleted official entry would come
            // back on every restart. Importing the official state successfully (adding and activating the
            // official entry) only hangs off the manual import command layer
    ...
    // Add the official entry while importing an existing config for the first time, matching the "first
    // launch = import default + seed the official entry" feel of the other apps. The grokbuild seed was
    // introduced after the `official_providers_seeded` flag, so main seeding no longer runs for existing
    // databases and it has to hang off the import action. Only runs on a successful import; a fully
    // unimportable live (missing file / syntax error / partial config) never gets here. Failures only warn.
    if matches!(app_type, AppType::GrokBuild) {
...
mod tests {
    ...
    fn codex_oauth_context_defaults_skip_non_gpt56_models() {
        ...
        let effective =
            build_effective_settings_with_common_config(&db, &AppType::Claude, &provider)
                .expect("build effective settings");
        // The old model injects no 372K default, and values dragged in by a legacy shared fragment are stripped too
        assert!(effective["env"]
            .get("CLAUDE_CODE_MAX_CONTEXT_TOKENS")
            .is_none());
        // Values the user wrote explicitly in the provider config still apply
        assert_eq!(
            effective["env"]["CLAUDE_CODE_AUTO_COMPACT_WINDOW"],
            json!("300000")
        );
    ...
    /// Round-trip fixed point: injected values only live in live; after switching away and backfilling,
    /// the stored config must match the pre-injection state, otherwise a default freezes as an "explicit value".
    #[test]
    fn codex_oauth_backfill_strips_injected_context_defaults() {
        ...
        // Simulate writing live: two context defaults were injected
        let live = build_effective_settings_with_common_config(&db, &AppType::Claude, &provider)
            .expect("build effective settings");
        assert_eq!(
        ...
        // Simulate switching away and backfilling: injected values are stripped, other fields kept as-is
        let backfilled =
            strip_common_config_from_live_settings(&db, &AppType::Claude, &provider, live);
        assert!(backfilled["env"]
            .get("CLAUDE_CODE_MAX_CONTEXT_TOKENS")
    ...
    fn codex_oauth_backfill_keeps_user_context_values() {
        ...
        // In live: MAX is an explicit user value; ACW was hand-edited by the user to a non-default number
        let live = json!({
            "env": {
                "ANTHROPIC_MODEL": "gpt-5.6",
                "CLAUDE_CODE_MAX_CONTEXT_TOKENS": "500000",
    ...
    /// C5 regression lock: the merge/strip of the frontend form must go through the toml_edit document model.
    /// Re-serializing the whole document with smol-toml's parse -> merge -> stringify loses comments,
    /// reorders keys alphabetically, and emits redundant empty parent table headers for dotted tables.
  ...
  "env": {
    ...
    fn category_less_managed_codex_binding_with_null_config_uses_selected_account_token() {
        ...
        // last_refresh is the write-time timestamp (non-deterministic), so assert field by field instead of whole-value equality.
    ...
    fn backfill_never_persists_native_tokens_into_managed_provider_config() {
        // The stored config of a managed provider uses a placeholder auth; but live right now holds the
        // user's own native browser-login auth (with a real refresh_token). backfill must swap the live
        // auth back to the stored placeholder and never write native access/refresh tokens into the DB config.
        let mut provider = Provider::with_id(
            "openai-official".to_string(),
            "OpenAI Official".to_string(),
            json!({
    ...
    fn backfill_leaves_non_managed_provider_auth_untouched() {
        // A non-managed provider is unaffected by this stripping: its auth must be kept as-is.
        let mut provider = Provider::with_id(
            "custom".to_string(),
            "Custom".to_string(),
  ...
  "allowedTools": ["tool1"]
    ...
    fn codex_switch_backfill_strips_synced_mcp_servers() {
        // [mcp_servers] in live is a projection of the MCP sync (SSOT lives in the DB table);
        // backfilling it into the provider's stored config would resurrect deleted servers with the snapshot.
        let provider = Provider::with_id(
            "prov".to_string(),
            "Prov".to_string(),
            json!({
