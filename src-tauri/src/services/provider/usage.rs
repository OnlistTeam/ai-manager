...
pub(crate) async fn execute_and_format_usage_result(
    ...
    .await
    {
        ...
        Err(err) => {
            // Transient transport failures (send/timeout, interrupted body read) propagate as Err so
            // the frontend invoke rejects -> react-query retries and keeps the last good value; decided
            // by error key, not message text. Other script/config/HTTP business errors collapse into
            // success:false and are shown as text.
            ...
            let msg = match err {
                AppError::Localized { message, .. } => message,
    ...
    if let Some(interval) = script.auto_query_interval {
        if interval > 1440 {
            return Err(AppError::localized(
                ...
                format!("Auto query interval cannot exceed 1440 minutes (24 hours), current: {interval}"),
            ));
        }
    }
