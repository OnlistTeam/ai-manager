...
/// Custom endpoint config (legacy compatibility; actually stored in provider.meta.custom_endpoints)
...
impl Default for VisibleApps {
    fn default() -> Self {
        Self {
            ...
            opencode: true,
            openclaw: true,
            hermes: false, // Hidden by default; the user must enable it manually
            pi: true,
        }
    }
}
...
/// WebDAV sync state (persisted sync progress information)
...
/// Per-machine auto-migration state.
///
/// Records the one-shot migrations this machine ran at startup; the markers are not synced with the database.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
#[serde(rename_all = "camelCase")]
pub struct LocalMigrations {
    ...
    /// Official-history migration marker for the unified session switch. Cleared when the
    /// switch is turned off, so turning it on again also migrates the official sessions
    /// that landed in the openai bucket "while it was off".
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct CodexOfficialHistoryUnifyMigration {
    ...
    #[serde(default)]
    pub migrated_state_rows: usize,
    /// The normalized Codex directory at migration time. The marker only applies to that
    /// same directory: after changing codex_config_dir, the old marker does not block the
    /// migration for the new directory.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
///
/// Holds device-level settings stored in the AI Manager product AppData and not synced
/// with the database, so multiple devices can work independently when cloud sync is used.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AppSettings {
    ...
    /// Whether the Claude plugin integration is enabled
    #[serde(default)]
    pub enable_claude_plugin_integration: bool,
    /// Whether to skip the first-install confirmation for Claude Code
    #[serde(default)]
    pub skip_claude_onboarding: bool,
    /// Whether to launch at login
    #[serde(default)]
    pub launch_on_startup: bool,
    /// Silent startup (do not show the main window at launch, run in the tray only)
    #[serde(default)]
    pub silent_startup: bool,
    /// Whether the local proxy feature is enabled on the home page (off by default)
    #[serde(default)]
    ...
    /// Current Claude provider ID (local storage, takes precedence over is_current in the database)
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub current_provider_claude: Option<String>,
    /// Current Claude Desktop provider ID (local storage, takes precedence over is_current in the database)
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub current_provider_claude_desktop: Option<String>,
    /// Current Codex provider ID (local storage, takes precedence over is_current in the database)
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub current_provider_codex: Option<String>,
    /// Current Gemini provider ID (local storage, takes precedence over is_current in the database)
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub current_provider_gemini: Option<String>,
    /// Current Grok Build provider ID (local storage, takes precedence over is_current in the database)
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub current_provider_grokbuild: Option<String>,
    /// Current OpenCode provider ID (local storage; may be meaningless for OpenCode but keeps the structure consistent)
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub current_provider_opencode: Option<String>,
    /// Current OpenClaw provider ID (local storage; may be meaningless for OpenClaw but keeps the structure consistent)
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub current_provider_openclaw: Option<String>,
    /// Current Hermes provider ID (local storage, keeps the structure consistent)
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub current_provider_hermes: Option<String>,

    // ===== Skill sync settings =====
    /// Skill sync method: auto (default, prefers symlink), symlink, or copy
    #[serde(default)]
    ...
    // ===== WebDAV backup settings (legacy, kept for backward compatibility) =====
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    // ===== Terminal settings =====
    /// Preferred terminal application (optional; the system default is used otherwise)
...
impl AppSettings {
    fn settings_path() -> Option<PathBuf> {
        // settings.json is kept for migrating older versions and for the no-database case
        Some(crate::infrastructure::paths::product_data_dir().join("settings.json"))
    }

    fn normalize_paths(&mut self) {
        // Never allow a saved preference to hide every recovery surface. Old
        // inherited settings are normalized before startup consumes them.
    ...
    fn load_from_file() -> Self {
        ...
        if let Ok(content) = fs::read_to_string(&path) {
            match serde_json::from_str::<AppSettings>(&content) {
                ...
                Err(err) => {
                    log::warn!(
                        "Failed to parse the settings file; using the defaults. Path: {}, error: {}",
                        path.display(),
                        err
                    );
                    Self::default()
...
fn save_settings_file(settings: &AppSettings) -> Result<(), AppError> {
    ...
    let Some(path) = AppSettings::settings_path() else {
        return Err(AppError::Config(
            "Unable to determine the user home directory".to_string(),
        ));
    };
...
/// Reload the settings from file into the in-memory cache
/// Used for scenarios such as config import, to keep the cache in sync with the file
...
// ===== Current provider management functions =====

/// Return the current provider ID for the given app type (read from the local settings)
///
/// This is a device-level setting that is not synced with the database.
/// When it is absent locally, callers should fall back to the `is_current` field in the database.
...
/// Set the current provider ID for the given app type (saved to the local settings)
///
/// This is a device-level setting that is not synced with the database.
/// Passing `None` clears the current provider setting.
...
/// Return the effective current provider ID (validating that it exists)
///
/// Logic:
/// 1. Read the current provider ID from the local settings
/// 2. Verify that the ID exists in the database
/// 3. If it does not, clear the local settings and fall back to is_current in the database
///
/// This guarantees the returned ID is valid (it exists in the database). With multi-device
/// cloud sync the local ID can become stale after a config import; this function repairs it.
pub fn get_effective_current_provider(
    db: &crate::database::Database,
    app_type: &AppType,
) -> Result<Option<String>, AppError> {
    // 1. Read from the local settings
    if let Some(local_id) = get_current_provider(app_type) {
        ...
        if providers.contains_key(&local_id) {
            // It exists, return it directly
            return Ok(Some(local_id));
        }

        // 3. It does not exist, clear the local settings
        log::warn!(
            "Provider {} ({}) from the local settings does not exist in the database; clearing it and falling back to the database",
            local_id,
            app_type.as_str()
        );
    ...
    // Fall back to is_current in the database
    db.get_current_provider(app_type.as_str())
}

// ===== Skill sync method management functions =====

/// Return the skill sync method setting
...
// ===== Skill storage location management functions =====

/// Return the skill storage location setting
...
/// Set the skill storage location
...
// ===== Backup policy management functions =====
...
// ===== Terminal settings management functions =====

// ===== WebDAV sync settings management functions =====

// ===== S3 sync settings management functions =====
...
mod tests {
    ...
    #[test]
    fn inherited_hidden_window_preferences_keep_a_recovery_surface() {
        let mut without_tray = AppSettings {
            show_in_tray: false,
            minimize_to_tray_on_close: true,
        ...
        without_tray.normalize_paths();
        assert!(!without_tray.minimize_to_tray_on_close);
        assert!(!without_tray.silent_startup);

        let mut without_auto_launch = AppSettings {
            launch_on_startup: false,
            silent_startup: true,
            ..AppSettings::default()
        };
        without_auto_launch.normalize_paths();
        assert!(!without_auto_launch.silent_startup);
    }
}
