...
describe("Codex TOML utils", () => {
  ...
  // P3 regression: must not mistakenly add a line when the config isn't using bearer token mode
  ...
  // P3 regression: clearing the API key in bearer mode must actually erase the token, so pickCodexApiKey's fallback can't find it
